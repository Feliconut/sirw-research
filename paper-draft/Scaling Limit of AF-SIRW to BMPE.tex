...
\begin{align*}\label{eq: transition prob for GPU}
	\mathbb{P} \left(\left(\mathcal{B}^{(y)}_{k+1},\mathcal{R}^{(y)}_{k+1} \right)=  (i+1,j) \vert \left(\mathcal{B}^{(y)}_{k},\mathcal{R}^{(y)}_{k}\right) =(i,j)  \right) &= \frac{b_y(i)}{b_y(i)+r_y(j)}, \mbox{ and}  \\
	\mathbb{P} \left( \left(\mathcal{B}^{(y)}_{k+1},\mathcal{R}^{(y)}_{k+1}\right)=  (i+1,j) \vert \left(\mathcal{B}^{(y)}_{k},\mathcal{R}^{(y)}_{k}\right) =(i,j)  \right) &= \frac{r_y(j)}{b_y(i)+r_y(j)},
\end{align*} 
...
% TBD 2 examples below
%\bibitem{KL}
%Kipnis,~C.,~Landim,~C.: Scaling limits of interacting particle systems. \emph{Grundlehren~der mathematischen Wissenschaften}, Berlin-Heidelberg-New York: Springer-Verlag, 1999. \MR{1707314}
%
%\bibitem{KV} Kipnis,~C., Varadhan~S.R.S.: Central limit theorem for additive functionals of reversible
%Markov processes and applications to simple exclusion. \emph{Commun. Math. Phys.} \textbf{106}, (1986), 1--19. \MR{834478}
