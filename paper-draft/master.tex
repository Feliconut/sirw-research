...
% % Modified  2023_09_06
% % Lastest Edition, 2023_09_06
% % Description of changes:
% %  Major changes are in subsection 3.2: 
...
\newcommand{\edt}[1]{\textcolor{red}{#1}} %edit time 09/6/2023
...
\begin{document}
	...
	\end{equation} 
	where $B \in \mathbb{R}$, $p \in (0,1)$ and $\kappa > 0$.
...
%		the accumulated local drifts at site $x$ by $m$-th visit, $$\delta_{x,m}:= \sum_{i=0}^\infty E[X_{i+1}-X_i\vert \mathcal{F}_{i}^X] \mathbb{1}_{\{X_i=x, L(x,i)\leq m\}};$$ 
	...
	Given a (recurrent) random walk $(X_n)_{n\geq 0}$, and a fixed site $y\in \mathbb{Z}$, we can obtain a Markov process by considering only the up-crossings and down-crossings of $X_n$ from site $y$. More precisely, let $(\lambda_{y,k})_{k\geq 0}$ be the stopping times when $X_n$ \edt{visit site $y$ for the $\left( i+1 \right) $-th time:}
	\[
		\lambda_{y,0} :=\inf\{ k\geq 0: X_t = y \} , \quad \lambda_{y,k+1} := \inf\{ t> \lambda_{y, i}: X_t = y \}.
	   \] 
...
\[
	...
	\begin{equation}\label{eq: initial condition}
		\left(L(\{y, y-1\},\lambda_{y,0}),  L(\{y, y+1\} ,\lambda_{y,0})\right) =  \begin{cases}	
			(1, 0) &,  \text{ if }  y>0 \\
			(0, 1) &,  \text{ if }  y<0 \\  
			(0, 0) &,  \text{ if }  y=0 \\
		\end{cases} 
	...
	\] 
	Summing over the terms between two consecutive stopping times $\tau^{B}_{l,y} $ and $\tau^{B}_{l+1,y} $, for any $0\leq l \leq k -1$,  we get a sum depending only on $\tau^{B}_{l,y} $, $\tau^{B}_{l+1,y} $, 
	...
	Therefore,  $\Delta_{y}^{(x,m)}$ is a sum of $\mathcal{E}^{(x,m)}_{y-1,+} $ increments which (jointly) depend only on $ \left(\tau^{B}_{l+1,y}: 0\leq l\leq   \mathcal{E}^{(x,m)}_{y-1,+}-1 \right) $
			...
			\end{equation}			
	From \eqref{eq: conditional increment}, $\abs{  D(\tau^{B}_{l,y},\tau^{B}_{l+1,y} )} \leq  \tau^{B}_{l+1,y}-\tau^{B}_{l,y}$ is stochastically dominated by a geometric random variable with a mean independent of $l$. 
	...
	\[
		...
		= \mathbb{E}\left[ \sum_{l=0 }^{ k -1  }  D(\tau^{B}_{l,y},\tau^{B}_{l+1,y} ) \vert \mathcal{E}^{(x,m)}_{y-1,+} \right]. 
	\]
	As $\left(\tau^B_{l,y}\right)_{l \geq 0} $ is independent of $ \mathcal{E}^{(x,m)}_{y-1,+} $,  $\left(D(\tau^{B}_{l,y},\tau^{B}_{l+1,y} )\right)_{l\geq 0}$ is also independent of $ \mathcal{E}^{(x,m)}_{y-1,+} $. Then, from \eqref{eq: conditional increment} again, on the event $\left\{\mathcal{E}^{(x,m)}_{y-1,+}  = k  \right \}$
	...
	\end{equation} 
	 Expressions like \eqref{eq: cummulated drift at a site} and \eqref{eq: conditional mean in GPU represenetation} help us simplify the analysis of $\Delta_{y}^{(x,m)}$ and $\rho_{y}^{(x,m)}$, as will be shown section \ref{sec: approximations}. To analyze events like $ L(y,\lambda_{x,m}) = k+k'$ and $\mathcal{E}^{(x,m)}_{y-1,+} = k $, the branching-like processes play a key role.
		...
		Having understood the behavior of the walk at a fixed site in terms of the urn processes, we now want to characterize how urn processes at different sites are related. As observed in \cite{KMP22}, if we know the number of upcrossings $n$ from site $y$ to $y+1$ before $\lambda_{x, m}$, it implies that the walk has made $n$ excursions to the right. The end of the $n$-th excursion coincides with the $n$-th left step from $y+1$ after  $\lambda_{x, 0}$. Therefore the directed edge local times at $y+1$ can be reconstructed by drawing from the local urn at $y+1$ until we hit the $n$-th step to the left. By iterating this process, we observe that the directed edge local times at $\lambda_{x,m}$ form Markov chains, and 
		\begin{align}
			\label{eqn:BLP-trans}
			P\left(\mathcal{E}_{y, +}^{(x,m)} = i \middle| \mathcal{E}_{y-1, +}^{(x,m)} = j \right) 
			&= P\left(  \mathcal{R}_{\tau_{j}^{\mathcal{B}}}^{(y)} = i \right)  
			&\text{whenever }y > x , \\
			P\left(\mathcal{E}_{y, -}^{(x,m)} = i \middle| \mathcal{E}_{y+1, -}^{(x,m)} = j \right) 
			&= P\left( \mathcal{B}_{\tau_{j}^{\mathcal{R}}}^{(y)}  = i \right) 
			&\text{whenever }y < x , \nonumber
	\end{align} 
	where the urn processes on the right are all located at $y$.

	\comment{This expression should not require assumption that $x>0$ since this is encoded in the different initial states of the urns at different locations.}
		
	In view of Remark~\ref{rem:symmetry}, we assume $x \ge 0$. We define the \textit{branching-like processes} to be
		\[
			\tilde{\zeta} := \left(\mathcal{E}^{(x,m)}_{x+k,+} \right)_{k\geq 0}, \quad
			\zeta := \left(\mathcal{E}^{(x,m)}_{x-k,-} \right)_{k\geq 0}
			.\]
			Their transition probabilities of $\tilde{\zeta}$ and $\zeta$ can be stated in terms of \eqref{eq: generalized weights}, using observation in \eqref{eqn:BLP-trans}. By observations above, they are both Markov chains. Moreover, $\tilde{\zeta}$ is homogeneous, i.e. its transition probabilities are independent of $k$. By contrast, transition probabilities of $\zeta$ depends on the sign of  $x - k$, corresponding to \eqref{eq: generalized weights}.
	
		We denote the natural filtrations of $\tilde{\zeta}$ and $\zeta$ as $\mathcal{G}_{y, +}^{(x,m)}:=\sigma\left(\mathcal{E}^{(x,m)}_{z, +}: x \le z \le y\right) $ for $y \ge x$ and $\mathcal{G}_{y, -}^{(x,m)}:=\sigma\left(\mathcal{E}^{(x,m)}_{z, -}: y \le z \le x\right) $ for $y \le x$. We may simply write $\mathcal{G}_y^{(x,m)}$ is clear from context.

		Now we evaluate the urns in a more refined order and construct a filtration that is term-wise finer than $\mathcal{G}_y^{(x,m)}$.
		We first construct a finer version of $\tilde{\zeta}= \left(\mathcal{E}^{(x,m)}_{x+k,+} \right)_{k\geq 0}$. Due to the strong Markov property of $X_n$, the generalized P\'{o}lya urn processes at different sites are independent, and the sequences $(\tau^B_{k,y})_{k\geq 0} $ are also independent in $y \geq x$. As each sequence $\left(\tau^B_{k,y}\right)_{k\geq 0} $ is Markov in $k$,  the collection of stopping times  
		 ...
		 under the lexicographical order: 
			\begin{equation}\label{eq: lexicographical order}
				(y,k) \preceq (y',k')  \mbox{ if and only if  }
					...
					y <y'. 
			\end{equation} 
		At time $\lambda_{x,m}$, for any $y\geq x$, the last jump from site $y+1$ is a left jump and it is the $\mathcal{E}_{y+1,-}^{(x,m)}=\mathcal{E}_{y,+}^{(x,m)}$ -th left jump, the number of right jumps from site $y+1$ is exactly
		...
		\end{equation}  
	   where $L = \mathcal{E}_{y+1,-}^{(x,m)}=\mathcal{E}_{y,+}^{(x,m)} $. With \eqref{eq: markov 1}, \eqref{eq: lexicographical order}, and \eqref{eq: recursive formula for upcrossings}, we conclude that $\tilde{\zeta} = \left(\mathcal{E}^{(x,m)}_{x+k,+} \right)_{k\geq 0}$ is a homogeneous Markov chain when $x\geq 0$, and its the transition probability is 
	   ...
	   \end{equation} 
   		...
   		{
   		On the other hand, the $\zeta= \left(\mathcal{E}^{(x,m)}_{x-k,-} \right)_{k\geq 0}$ follows a similar arguments. We get similar statements to \eqref{eq: markov 1} and \eqref{eq: recursive formula for upcrossings}:
   		\begin{enumerate}
   			\item 
   			...
   			\begin{equation}\label{eq: markov 2} 
   				\left\{\tau^R_{k,x-y}: y\geq x, k\geq 0 \right\} \mbox{are Markov in $(y,k)$}
   		...
   		\end{enumerate}	
   	 Therefore, from \eqref{eq: markov 2} and \eqref{eq: recursive formula for downcrossings}, we get that $\zeta= \left(\mathcal{E}^{(x,m)}_{x-k,-} \right)_{k\geq 0}$ is an inhomogeneous Markov chain, with the transitional probability  depending on the sign of $x-k$: for $i,j\geq 0$
...
\TBD \textcolor{blue}{The \eqref{eq: transition prob on negative} is slightly different from (6) in \cite{KP16}.}

We define our second filtration
\[
	\mathcal{H}_{y, +}^{(x,m)} = \sigma\left(\left\{\mathcal{F}_{m}^{\mathcal{B}, \mathcal{R}^{(x)}}\right\}  \cup \left\{\tau_{k, z}^{B} : x < z \le y, 0 \le k \le \mathcal{E}_{z, -}^{(x,m)} \right\} \right) 
.\] 
$\mathcal{H}_{y, -}^{(x,m)}$ is defined similarly. We drop the $+, -$ signs when there is no ambiguity. Now, observe that 
\begin{enumerate}
	\item 
...
\end{lemma}
\begin{lemma}
	On $G_{n, K, t}$, $\sum_{y \ge x} \left( \Delta_y^{(x,m)} - \tilde\rho_y^{(x,m)} \right) $ is Lipchitz with some Lipchitz constant depending on $K, p$ and $n$. Note, however, that it is not a martingale.
\end{lemma}
\begin{proof}
	The previous lemma provides a control for all $\Delta_y^{(x,m)} - \rho_y^{(x,m)}$. Hence we need to control the difference $\rho_y^{(x,m)} - \tilde \rho_y^{(x,m)}$. 
	\begin{align*}
		\rho_y^{(x,m)} - \tilde \rho_y^{(x,m)}
		&= \mathbb{E}\left[ \Delta_y^{(x,m)} - \Delta_{y}^{(x,m)}\mathbf{1}\left( G_{n, K}^{(x,m)}(y) \right) \middle| \mathcal{H}_{y-1}^{(x,m)}  \right]  \\
		&= \mathbb{E}\left[ \Delta_y^{(x,m)} \mathbf{1}\left( \left( G_{n, K}^{(x,m)}(y) \right) ^c \right) \middle| \mathcal{H}_{y-1}^{(x,m)} \right] 
	.\end{align*}
	Hence
	\begin{align*}
		\left| \rho_y^{(x,m)} - \tilde \rho_y^{(x,m)} \right| 
		&\le \sqrt{
			\mathbb{E}\left[ \left( \Delta_y^{(x,m)} \right) ^2 \middle| \mathcal{H}_{y-1}^{(x,m)} \right] P\left( \left( G_{n, K}^{(x,m)}(y) \right) ^c  \right)  
		}
	.\end{align*}
	On $G_{n,K, t}$, \TBD
\end{proof}
...
\begin{equation}
	...
	\sum_{y \ge x} \left( \Delta_y^{(x,m)} - \tilde\rho_y^{(x,m)} \right) \mathbf{1}\left( G_{n, K}^{(x,m)}(y) \right)  \mathbf{1}\left(|y| < K \sqrt{n}\right) 
\end{equation}
