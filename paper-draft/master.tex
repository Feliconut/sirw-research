...
\end{equation} ,
	...
	\begin{equation}\label{eq: QV term}
		\lim_{n\to \infty}\frac{1}{n} \sum_{i=0}^{n-1}\mathbb{E}\left[ (M_{i+1}- M_{i})^2 |\mathcal{F}_i^X \right] =1 \qquad  \mbox{ in probability}.
	\end{equation}
	...
	\begin{lemma}\label{lm: control of acc drift}
		...
		\end{equation}
	\end{lemma}
	Before proving Lemma~\ref{lm: control of acc drift}, we first remark that the local time process of $X_k$ at excursion times $\lambda_{x,m}$ can be described in terms of spacially markovian processes, namely branching-like processes discussed in section \ref{sec: generalized Polya Urn, BLP}. This motivates considering \textit{local drift} attained at a single site $y$ at stopping times $\lambda_{x, m}$:
