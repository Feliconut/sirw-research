...
% % (0) Minor updates
% % (1) Major updates on Section 4.1. 
% %	(2) Slight modifications on section 3   
% % 
% % Changes to be made:  
% % (1) Motivations for 2.2 for 2.2; 
% % (2) rework of descriptions after proposition 2.2, especially a focus on lambda_{x,m} instead of time k: 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{document}
	...
	\end{equation} 
	for some $p \in (0,1]$, $\kappa>0$ and $B\in \mathbb{R}$.
...
%	\edt{Consider changing both propositions to a version for $(x,m)$. This allows us to unify the part of the proof where we go from $(X_k, L(X_k, k))$ to $(x,m)$.}
	...
	Both processes can be obtained from the SIRW $(X_n)_{n\geq 0}$ at stopping times. We start with the generaized P\'{o}lya urn processes. 

	\subsection{Generalized P\'{o}lya  Urn}
	Given a (recurrent) SIRW $(X_n)_{n\geq 0}$, and a fixed site $y\in \mathbb{Z}$, we can obtain a Markov process by considering only the up-crossings and down-crossings of $X_n$ from site $y$. More precisely, let $(\lambda_{y,k})_{k\geq 0}$ be the stopping times when $X_n$ visit site $y$ for the $\left( k+1 \right) $-th time:
	\[
		\lambda_{y,0} :=\inf\{ t\geq 0: X_t = y \} , \quad \lambda_{y,k+1} := \inf\{ t> \lambda_{y, k}: X_t = y \}.
	   \] 
	\edt{This is consistent with the notation in point 3 of subsection 1.4. }
	  ...
	  \begin{equation} \label{eq: RW to GPU}
	   ...
	   Intuitively, $\left(\mathcal{B}_{i}^{(y)},\mathcal{R}_{i}^{(y)} \right)$ is the state of a (generalized) P\'olya urn, after the $i$-th draw made from the urn. This is the reason that we define $\lambda_{y, 0}$ to be the first time the walk reaches $y$ : this is right before the first draw be made from the urn. 
	\begin{remark}
		\label{rem:symmetry}
...
\[
	...
	\]  
		...
		\]
		are two Markov processes on $\mathbb{N}\cup\{0\}$, whose transition probabilities are related to the generalized weights in \eqref{eq: generalized weights}, summarized in \eqref{eq: transition prob on positive} and \eqref{eq: transition prob on negative} below. The derivation is known in several earlier works, such as \cite{T96, KP16, KMP22}. We state some facts in the derivation of \eqref{eq: transition prob on positive} and \eqref{eq: transition prob on negative} , which is also used in the next subsection.
		...
		\begin{enumerate}
			\item Although the generalized P\'{o}lya urn processes at different sites are not independent, the sequences $(\tau^B_{k,y})_{k\geq 0} $ are independent in $y \geq x$, and each sequence $\left(\tau^B_{k,y}\right)_{k\geq 0} $ is Markov in $k$. Then, the collection of stopping times  
...
The following lemma says that $\Delta^{(x,m)}_{y}$ is $\mathcal{H}_{y, +}^{(x,m)}$- measurable but not $\mathcal{H}_{y, +}^{(x,m)}$- measurable, and we have two identities for $\Delta^{(x,m)}_{y}$ and  $\rho_{y}^{(x,m)}$. The argument is standard, \TBD \edt{and works for any $x\in \mathbb{Z}$.}
...
\begin{proof}%[Proof of Lemma~\ref{lm:lipchitz-bound-on-good-event}]
	We verify this lemma for the case $y \ge  x > 0$. The cases $x = 0$ and $x<0$ bring slight difference in calculation but does not affect the bound.
\begin{align*}
...
Using process-level tightness, the summation only add a term of order $\sqrt{N}$, on a good event. Since we have a factor of $\frac{1}{N}$, and $M$ independent of $N$, we have the right hand side $\to 0$. More precisely, take $G = G_{n, K, t}$ and
\begin{align*}
...
On $G_{n, K, t}$, \eqref{eqn:tempered-difference-1} is zero. Proposition~\ref{lm: approximation of means of local drift} has controlled \eqref{eqn:tempered-difference-4}. 
...
To control the first probability, write $l = L\left( x+1, \lambda_{x, m} \right) $, and note that
\[
...
.\] 
Use the observation that Generalized Polya urn processes are stochastically bounded by geometric random variables, 
\begin{align*}
...
Having exponentially decaying probability of good event, we only need a naive bound for the second expectation.
