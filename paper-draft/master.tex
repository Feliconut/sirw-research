...
% % Modified  2023_12_13
% % Lastest Edition, 2023_12_13
% % Description of changes:
% % (1) Minor changes in abstract.
% % 
% % Changes to be made:  
% % (1) Issues in Introduction
% % (1.1) Ray-knight Approaches/or not
% % (1.2)
% % (2) Abstract
...
%We consider a one-dimensional non-Markovs self-interacting random walk, and complete the functional limit theorem in \cite{KMP23} for the asymptotically free case with $0<p \leq \frac{1}{2}$. The approach is to carefully approximate the local drifts of the random walk via studying the directed edge local times, described by branching-like processes and generalized Ray-Knight Theorems. It turns out that certain martingales associated to these branching-like processes are well behaved. (Or we construct events such that certain martingales associated to these branch-like processes are well behaved.)
	...
	We consider a discrete time nearest neighbor self-interacting random walk (SIRW) $(X_k)_{k\geq 0}$ on $\mathbb{Z}$ as in \cite{T96,KMP23}. The walk $(X_k)_{k\geq 0}$ starts from $X_0 = 0$ and is non-Markov: its transition probability for $X_{k+1}$ depending on its history up to $X_k$, 
	\begin{align}
	...
	This model was first studied by B. T\'oth in \cite{T96}, according to whose terminology, the SIRWs with weight functions of the form \eqref{eq: asymptotics of w} fall into the ``asymptotically free'' case: $w(n)\sim 1$, while the SIRW with weight functions $w(n)\sim n^{-\alpha}$, for some $\alpha >0$, corresponds to ``polynomially self-repelling'' case. In \cite{T96}, T\'oth proved for both cases functional limit theorems for the local time processes of SIRWs, 
	...
	Based on these result, in \cite{KMP23}, Kosygina, Mountford and Peterson showed the process-level tightness of extrema process of walk, 
	...
	\edt{More discussion here about the Ray-Knight type of approach in establishing functional convergence to BMPE.}
	...
	Discuss related result to the functional convergence to BMPE, and the Ray-Knight type of approach. Discuss roughly which conditions/models are "stable" having this result, and which ones don't or having technical difficulties.
	 ...
	 4. Ray-Knight type of approach is very powerful and promising in deriving (some of) the above mentioned properties and deriving properties of the local time profile is a good starting point for inferring information of original walk.  \\
	 5. Recurrence of the SIRW is comparable to non-boundary recurrent case of ERW. In the boundary case the ERW has a different scaling. %RN is easier to use when walk is transient, invert hitting times of random walks" from Markov paper. 
 }
...
.\]
	...
	\begin{lemma}(Proposition A.3 \cite{KMP23})\label{lm: diffusion approximation of blp}
		...
		\] 
		as $n$ goes to infinity on $D([0,\infty)),$ where $Z^{(2-2\gamma)}(.)$ is $\frac{1}{2}$ times the squared Bessel processes of dimension $2-2\gamma$.
