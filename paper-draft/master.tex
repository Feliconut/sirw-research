...
On $G_{n, K, t}$, we obtain priori bounds \comment{a priori?} of local drifts, which allows us to apply martingale concentration inequalities to estimate accumulated drift.
...
\begin{proof}%[Proof of Lemma~\ref{lm:lipchitz-bound-on-good-event}]
	...
	\begin{align*}
		...
		\right| \\
		\intertext{From the identity $2\frac{a - b}{a + b} = (\frac{1}{a} - \frac{1}{b})(\frac{1}{a} + \frac{1}{b})^{-1}$, and the fact that $w(.)$ is bounded, this is controlled by}
		...
		&\le C_w \left(4 \log^4 n + \sum_{i = 4 \log^4 n}^{K \sqrt{n} } \log^2 n \sup_{|j - i| \le 2 \sqrt{i}  \log^2 n} \left|  2^p B\left( \frac{1}{(2j)^p} - \frac{1}{(2i + 1)^p} \right)  + O\left( \frac{1}{i^{\kappa + 1}} \right) \right|\right)\\
		&\le C_{w, p} \sum_{i = 0}^{K \sqrt{n} } \log^2 n \left( 
		\frac{4 \sqrt{ i } \log^2 n }{\left(2 i - 2 \sqrt{ i } \log^2 n\right)^{p + 1}} + \frac{1}{\left(2 i\right)^{p + 1}} + O\left( \frac{1}{i^{\kappa + 1}} \right)
		\right)  \\
		&\le C_{w, p} \sum_{i = 0}^{K \sqrt{n} } i^{- p - \frac{1}{2}} \log^4 n + i^{- \kappa - 1 } \log^2 n
