...
\[
	...
	Since the process $(\tau^B_{l,y})_{l\geq 0}$ is Markov, and  $ (\mathcal{E}^{(x,m)}_{y})_{ y \geq x} $ is also Markov from the following subsection \TBD, we get that, on the event that $ \left\{ \mathcal{E}^{(x,m)}_{y-1,+} = k \right\} = \left\{ \mathcal{E}^{(x,m)}_{y,-} = k  \right\}$, $ \rho_{y}^{(x,m)} $ equals
	$$
	...
	\end{equation} 
	 Expressions like \eqref{eq: cummulated drift at a site} and \eqref{eq: conditional mean in GPU represenetation} help us simplify the analysis of $\Delta_{y}^{(x,m)}$ and $\rho_{y}^{(x,m)}$, as will be shown section \ref{sec: approximations}. To analyze events like $ L(y,\lambda_{x,m}) = k+k'$ and $\mathcal{E}^{(x,m)}_{y-1,+} = k $, the branching-like processes play a key role.
		...
		\begin{align}
			\label{eqn:BLP-trans}
			\mathbb{E}\left[\mathcal{E}_{y, +}^{(x,m)} \middle| \mathcal{E}_{x, +}^{(x,m)},\ldots, \mathcal{E}_{y-1, +}^{(x,m)} = j \right]
			&=\mathbb{E}\left[\mathcal{E}_{y, +}^{(x,m)} \middle| \mathcal{E}_{y-1, +}^{(x,m)} = j \right] = \mathcal{R}_{\tau_{j}^{\mathcal{B}}} 
			&\text{whenever }y > x , \\
			\mathbb{E}\left[\mathcal{E}_{y, -}^{(x,m)} \middle| \mathcal{E}_{x, -}^{(x,m)},\ldots, \mathcal{E}_{y+1, +}^{(x,m)} = j \right]
			&=\mathbb{E}\left[\mathcal{E}_{y, -}^{(x,m)} \middle| \mathcal{E}_{y+1, -}^{(x,m)} = j \right] = \mathcal{B}_{\tau_{j}^{\mathcal{R}}} 
			&\text{whenever }y < x , \nonumber
	\end{align} 
	...
	In view of Remark~\ref{rem:symmetry}, we assume $x \ge 0$. We define the \textit{branching-like processes} to be
		...
		We first construct a finer version of $\tilde{\zeta}= \left(\mathcal{E}^{(x,m)}_{x+k,+} \right)_{k\geq 0}$. Due to the strong Markov property of $X_n$, the generalized P\'{o}lya urn processes at different sites are independent, and the sequences $(\tau^B_{k,y})_{k\geq 0} $ are also independent in $y \geq x$. As each sequence $(\tau^B_{k,y})_{k\geq 0} $ is Markov in $k$,  the collection of stopping times  
...
\begin{lemma}
	...
	\[
		...
		P(G_{n, K,t}) = 1
	.\] 
	And if we denote \eqref{eqn:good-event-1} by $A_{n, K, t}$, then
	\[
...
\begin{equation}
	...
	\sum_{y \ge x} \left( \Delta_y^{(x,m)} - \rho_y^{(x,m)} \right) \mathbf{1}\left( G_{n, K}^{(x,m)}(y) \right)  \mathbf{1}\left(|y| < K \sqrt{n}\right) 
\end{equation}
...
To bound $\sup_{k < nt} \left| \sum_{y > X_k} (\Delta_y^{(X_k,L(X_k, \left\lfloor nt \right\rfloor)} - \rho_y^{(X_k,L(X_k, \left\lfloor nt \right\rfloor)}) \right|$, we can do a supremum bound on all possible combinations of $(X_k, L(X_k, \left\lfloor nt  \right\rfloor)$. Thus, this amounts to bounding the supremum of $\sum_{y > x}\left| \Delta_y^{(x,m)} - \rho_y^{(x,m)} \right| $ over all $(x,m)$ pairs. Also note that, by Lemma~\ref{lem:good-event}, $G_{n, K}^{(x,m)}(y)$ is true for all $y \ge x$ on the event $G_{n, K, t}$.
\begin{align*}
	& P\left( \sup_{k < nt} \left| \sum_{y > X_k} (\Delta_y^{(X_k,L(X_k, \left\lfloor nt \right\rfloor)} - \rho_y^{(X_k,L(X_k, \left\lfloor nt \right\rfloor)}) \right| > \varepsilon \sqrt{n}  \right) \\
	&\le P(G_{n, K, t}^c) + P\left( \sup _{|x|, m < K \sqrt{n} } \left| \sum_{y > x} \left(\Delta_y^{(x,m)} - \rho_y^{(x,m)}\right) \right|  > \varepsilon \sqrt{n} , \bigcap_{y > x} G_{n, K}^{(x,m)}(y) \right) \\
