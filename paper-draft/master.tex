...
\begin{document}
	...
	\begin{abstract}
	We show the convergence of one-dimensional self-interacting random walks to a Brownian motion perturbed at extrema under the diffusive scaling. This completes the functional limit theorem in \cite{KMP23} for the asymptotically free case when $0<p \leq \frac{1}{2}$. The approach is to approximate drifts experienced by the walker at each site via studying the directed edge local time processes in align with the generalized Ray-Knight approach.
...
In the case $p \in (\frac{1}{2}, 1]$, the total drift accumulated at any site $x$ turns out to be absolutely summable, which allows one to estimate the total drift easily (\cite{KMP23}, Lemma~2.3-2.4).
	...
	\]
		...
		.\]
		We write $\mathcal{E}^{(x,m)}_{y, -}$ for $\mathcal{E}^{\lambda_{x,m}}_{y,-}$, and $\mathcal{E}^{(x,m)}_{y, +}$ for $\mathcal{E}^{\lambda_{x,m}}_{y,+}$. \comment{Note that for $x\neq 0$, $\lambda_{x,0} > 0$.}
	\end{enumerate}
	...
	For each $k\leq nt$, we can let $x = X_k$ and $ m +1=L(x,k)$ so that $k = \lambda_{x,m}$. From the decomposition \eqref{eq: drift in terms of local drifts} of $\Gamma_k$, we see that
	...
	\[
	\mathcal{H}_{y, +}^{(x,m)} = \sigma\left( \mathcal{E}_{z, -}^{(x,m)}, \tau_{k, z}^\mathcal{B}\cdot \mathbb{1}_{\{ k\leq \mathcal{E}_{z, -}^{(x,m)} \}} : x \leq  z \leq y,  k \geq 0 \right) 
	\]
	...
	\begin{lemma}\label{lm: identities for Del, rho} 
		For any integers $m,y\geq x$ with $m\geq 0$, the conditional expectation $\Delta^{(x,m)}_{y}$ depends only on $ \mathcal{E}_{y-1,+}^{(x,m)} = \mathcal{E}_{y,-}^{(x,m)} + \mathbb{1}_{ \{ 1\leq y \leq x \} }$  and $ \tau^{\mathcal{B}}_{l,y}$ for $l\leq \mathcal{E}^{(x,m)}_{y,-} $,
		...
		\end{equation} 
		on the event that $\mathcal{E}^{(x,m)}_{y-1,+}  = k + \mathbb{1}_{\{1\leq y\leq x\}}.$
	\end{lemma}  
	\begin{proof} 
		...
		$\left\{ \mathcal{E}^{(x,m)}_{y-1,+} =k +  \mathbb{1}_{\{1\leq y\leq x\}}\right\}$ equals $\left\{  L(y,\lambda_{x,m}) = \tau_{k,y}^{\mathcal{B}} \right\}, $ on which
		\[
...
.\]
	...
	The third result is about the diffusion approximations of the branching-like processes, and it is a consequence of Proposition A.3 in \cite{KMP23}. Its proof follows arguments from Toth \cite{T96}. A consequence of this result is the process level tightness of extrema, Proposition 2.1 \cite{KMP23}. 
...
\begin{proof}[Proof of Lemma~\ref{lm: control of martingale}]
	...
	$$\rho^{(x,m)}_y = E\left[\mathcal{D}_{\tau_l^{\mathcal{B}}}^{}\right].$$ 
