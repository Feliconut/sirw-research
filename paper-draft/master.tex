...
% % Modified  2023_11_26
% % Lastest Edition, 2023_12_05
% % Description of changes:
% % (1) Minor changes in Intro 
% % 
% % Changes to be made:  
...
\begin{document}
	...
	\begin{abstract}
			We consider a one-dimensional non-Markovs self-interacting random walk, and complete the functional limit theorem in \cite{KMP23} for the asymptotically free case with $0<p \leq \frac{1}{2}$. The approach is to carefully approximate the local drifts of the random walk via studying the directed edge local times, which are described by branching-like processes and generalized Ray-Knight Theorems. It turns out that certain martingales associated to branch-like processes are well behaved. (Or we construct certain events on which martingales associated to branch-like processes are well behaved.)
	\end{abstract}
