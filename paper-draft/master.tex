...
\] 
	...
	\begin{align}\label{eq: transition prob for GPU}
		\mathbb{P} \left( \left(\mathcal{B}_{i+1},\mathcal{R}_{i+1}\right)=   \left(\mathcal{B}_{i}+1,\mathcal{R}_{i}\right) \right)     &= \frac{w^-(i)}{w^-(i)+w^+(i)}, \mbox{ and}  \\
		\mathbb{P} \left( \left(\mathcal{B}_{i+1},\mathcal{R}_{i+1}\right)=   \left(\mathcal{B}_{i},\mathcal{R}_{i}+1\right) \right)     &= \frac{w^+(i)}{w^-(i)+w^+(i)},
	\end{align} 
	...
	The transition probabilities can be written explicitly as:
	\comment{here the use of $i, j, k$ are different}
\begin{align*}
