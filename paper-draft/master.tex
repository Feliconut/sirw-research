...
%\end{enumerate}	
		...
		In view of the generalized P\'{o}lya urn process (associated to a site $y> x$), (\comment{maybe $ y>x>0 $ currently only one side is dealt}) on the event that \edt{$ L = \mathcal{E}^{(x,m)}_{y-1,+} +\mathbb{1}_{\{1\leq y\leq x}\}$}, we have \eqref{eq: conditional mean in GPU represenetation} 
