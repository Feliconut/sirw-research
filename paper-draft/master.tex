...
\begin{document}
	...
	In \eqref{eq: control of martingale difference for local drift}, for any fixed $(x,m)$ with $\abs{x},m < K\sqrt{n}$, the sum $\sum_{z>x}^{y} \left( \Delta_z^{(x,m)} - \rho_z^{(x,m)} \right) $ is a martingale (indexed by $y$). To control the sum, we compare the martingale $\sum_y \Delta_y^{(x,m)} - \rho_y^{(x,m)}$ to a tempered version $\sum_{y > x} \tilde \Delta_{y}^{(x,m)} - \tilde\rho_y^{(x,m)}$ that has bounded increments on the good event $G_{n, K, t} \cap \{\lambda_{x,m} \leq nt \}$, where
	\[
...
%		Lemma~\ref{lm: approximation of means of local drift} has controlled \eqref{eqn:tempered-difference-4}. 
		...
		To control \eqref{eqn:tempered-difference-2} on $G_{n, K, t} \cap \{\lambda_{x,m} \leq nt \}$, Azuma's inequality and Lemma~\ref{lm:lipchitz-bound-on-good-event} imply for arbitrary $\varepsilon>0$,
		\begin{align}
		...
		= P\left(\mathcal{B}^{(x+1)}_{l} > \frac{K^2 \sqrt{n} }{2} \sqrt{n} \middle| \mathcal{R}^{(x + 1)}_l = K \sqrt{n}  \right)
		.\] 
		As increments $\tau^B_{l+1,y} -\tau^B_{l,y}$ of the Generalized  P\'{o}lya urn processes are stochastically bounded by independent geometric random variables with parameter uniform in $l\geq 0$ and $y \in \mathbb{Z}$, 
		\begin{align*}
	...
	For $\sup_{k < nt} \left| \sum_{y > X_k} \Delta_y^{\left(X_k,L(X_k, k)\right)} - \rho_y^{\left(X_k,L(X_k, k)\right)} \right|$, we use a union bound by considering possible values $(x,m)$ for $(X_k, L(X_k, \left\lfloor nt  \right\rfloor)$. Note that if $G_{n,k,t}$ occurs, for any $k\leq nt$, there is a pair $(x,m)$ with $\abs{x},m <K\sqrt{n}$ and $\lambda{x,m}=k$. Therefore, we get from \eqref{eqn:goodgood} that 
	\begin{align*}
		...
		\\
		&\le P(G_{n, K, t}^c) + P\left( \bigcup_{|x|, m < K \sqrt{n} } \left\{  \left| \sum_{y > x} \Delta_y^{(x,m)} - \rho_y^{(x,m)} \right|  > \varepsilon \sqrt{n} \right\}, \bigcap_{y > x} G_{n, K}^{(x,m)}(y) , \{ \lambda_{x,m} \leq nt\} \right) \\
		&\le P(G_{n, K, t}^c) + K^2 n \sup _{|x|, m \le  K \sqrt{n} }
		P\left( \left| \sum_{y \ge x} \Delta_y^{(x,m)} - \rho_y^{(x,m)} \right|  > \varepsilon \sqrt{n} , \bigcap_{y > x} G_{n, K}^{(x,m)}(y)  \right) \\
