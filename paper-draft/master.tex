...
\[
	...
	In view of Remark~\ref{rem:symmetry}, we assume $x \ge 0$. We define the \textit{branching-like processes} to be
		...
		Now we evaluate the urns in a more refined order and construct a filtration that is component-wise finer than $\mathcal{G}_y^{(x,m)}$.
		We start from $\tilde{\zeta}= \left(\mathcal{E}^{(x,m)}_{x+k,+} \right)_{k\geq 0}$ first. Due to the strong Markov property of $X_n$, the generalized P\'{o}lya urn processes at different sites are independent, and the sequences $(\tau^B_{k,y})_{k\geq 0} $ are also independent in $y \geq x$. As each sequence $(\tau^B_{k,y})_{k\geq 0} $ is Markov in $k$,  the collection of stopping times  
