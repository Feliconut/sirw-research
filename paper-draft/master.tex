...
% % Modified 2024_01_16
% % Lastest Edition, 2023_1_16

% % (1) Minor changes in the abstrac and intro
% % (2) grammatic changes 
% % (3) small changes in subsection 4.2
% % 
% % Changes to be made:  
...
%	This approach has also been used previously in some one-dimensional ERWs, mainly \cite{DK12} and \cite{KP16}.
	Then our approach is works on one-dimensional ERWs, such as \cite{DK12} and \cite{KP16}.
	...
	We denote by $\mathbb{P}$ the probability measure on $D([0,\infty))$ induced by $(X_k)_{k\geq 0}$, and by $\mathbb{E}$ the expectation with respect to $\mathbb{P}$. This is the only probability measure we are working with, since we have deterministic environment in this model. 
%	For a precise definition of the probability measures and related $\sigma$-algebras, see \cite{KMP23}.
	...
	\begin{proof}
		...
		\begin{align*}
			...
			&\le \mathbb{P}\left( 2 \sum_{y \ge 0} \mathcal{E}_y^{(0,m_{n, K})} \le n \right) + \mathbb{P}\left( \sup _{y \ge 0} \mathcal{E}_y^{\left( 0,m_{n, K} \right) }   > \frac{K \sqrt{n} }{2}  \right)+ \mathbb{P}\left( L(0,\lambda_{0, m_{n, K}}) > K \sqrt{n}  \right)  \\
			&\le 
	...
	\begin{proof}%[Proof of Lemma~\ref{lm:lipchitz-bound-on-good-event}]
		We present the proof of this lemma for the case $y \ge  x > 0$. The cases $x = 0$ and $x<0$ bring slight differences in calculation but identical bound. For simplicity of notation, we drop $y$ from the P\"olya's urn processes. We first use \eqref{eq: cummulated drift at a site} to write 
		%\edt{comment about $y$}.
		\begin{align*}
...
%			+ \frac{1}{\left(2 i\right)^{p + 1}} 
	...
	\begin{proof}[Proof of Lemma~\ref{lm: control of martingale}]
		...
		\begin{align*}
			...
			&\le \mathbb{P}\left(  \left| \sum_{|x| \le K \sqrt{N} } (C_{w, p, M} \log^9 N +M ) \right| > \varepsilon  N  \right) + \mathbb{P}\left( G_{N, K, 1}^c \right)  \\
			&\le \mathbb{P}\left(  C_{w, p, M, K} \, \sqrt{N} \log^9 N > \varepsilon  N  \right) + \mathbb{P}\left( G_{N, K, 1}^c \right) 
			.\end{align*}
