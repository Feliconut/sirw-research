...
% % Modified  2023_10_27
% % Lastest Edition, 2023_10_27
% % Description of changes:
% % (1) Updates according to Tom's comments 
% % (2) Minor updates for the proofs in section 4
% % 
% % Changes to be made:  
% % (0) pages 
...
\begin{document}
	...
	We consider a discrete time nearest neighbor self-interaction random walk $(X_i)_{i\geq 0}$ on $\mathbb{Z}$ as in \cite{T96,KMP22}. More precisely, let $w(.)$ be a monotone positive function on $\mathbb{N}_0$ satisfying
	...
	The SIRW $(X_i)_{i\geq 0}$ has the following dynamics, 
	...
	\begin{definition}
		Let $\theta_+, \theta_- <1$. A Brownian motions perturbed at
		extrema (BMPE) $W^{\theta_+, \theta_-} = \left(W^{\theta_+, \theta_-}_t, t\geq 0\right)$ with parameter  $(\theta_+, \theta_-)$ is the pathwise unique solution of the equation
		$$
	...
	\begin{enumerate}
		...
		\item the running maximum of $(X_n)$ at time $n$, $S_n= \sup\{y: L(y,n)\geq 1 \} $; the running minimum at time $n$, $I_n= \inf\{y: L(y,n)\geq 1 \} $;
...
%	\edt{Consider changing both propositions to a version for $(x,m)$. This allows us to unify the part of the proof where we go from $(X_k, L(X_k, k))$ to $(x,m)$.}
	...
	Given a (recurrent) SIRW $(X_n)_{n\geq 0}$, and a fixed site $y\in \mathbb{Z}$, we can obtain a Markov process by considering only the up-crossings and down-crossings of $X_n$ from site $y$. More precisely, we first let $(\lambda_{y,k})_{k\geq 0}$ be the stopping times when $X_n$ visit site $y$ for the $\left( k+1 \right) $-th time:
	\[
...
.\] 
As increments $\tau^B_{l+1,} -\tau^B_{l,x_1}$ of the Generalized Polya urn processes are stochastically bounded by geometric random variables with parameter uniformly in $l\geq 0$ and $y \in \mathbb{Z}$, 
\begin{align*}
...
\TBD
Having exponentially decaying of good events, we only need to bound the second moments,
\begin{align*}
