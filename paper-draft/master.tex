...
% % Major changes are in section 3.
% % 1.1) descriptions in subsection 3.1 
% %	1.2) addition of subsction 3.3, which consists of Delta and related sigma filed. motivations after constructions   
% % 
% % Changes to be made:  
...
\[
	...
	\begin{align*}\label{eq: transition prob for GPU}
		...
		\mathbb{P} \left((\mathcal{B}^{(y)}_{k+1},\mathcal{R}^{(y)}_{k})=  (i+1,j) \vert (\mathcal{B}^{(y)}_{k},\mathcal{R}^{(y)}_{k}) =(i,j)  \right) &= \frac{r_y(j)}{b_y(i)+r_y(j)},
	\end{align*} 
	...
	\]  
		...
		\]
		are two Markov processes on $\mathbb{N}\cup\{0\}$, whose transition probabilities are related to the generalized weights in \eqref{eq: generalized weights}, summarized in \eqref{eq: transition prob on positive} and \eqref{eq: transition prob on negative} below. The derivation is known in several earlier works, such as \cite{T96, KP16, KMP22}. We state some facts in their derivation, which is also used in the next subsection.
		...
		\begin{enumerate}
			...
			\item  From \eqref{eq: markov 1}, and \eqref{eq: recursive formula for upcrossings}, the transition probabilities for $\tilde{\zeta}$ are 
...
the second type of filtration contains additional information of all arrival times $\tau^B_{k,y}$ for all $k\leq \mathcal{E}^{(x,m)}_{y, -}$ and $\mathcal{E}^{(x,m)}_{y, -}$,
\[
...
In particular, $\mathcal{H}_{y, +}^{(x,m)}$ is finer than $\mathcal{G}_{y, +}^{(x,m)}$ because $\mathcal{E}_{y+1, -}^{(x,m)} = \mathcal{E}_{y, +}^{(x,m)}$ is  $\mathcal{H}_{y, +}^{(x,m)}$- measurable for any $y\geq x$ by \eqref{eq: recursive formula for upcrossings} and \eqref{eq: source of inhomogeneity} , and similarly, $\mathcal{E}_{y, -}^{(x,m)}$ is $\mathcal{H}_{y, -}^{(x,m)}$- measurable for $ y\leq x$. 

The following lemma discusses whether $\Delta^{(x,m)}_{y}$ and $\rho^{(x,m)}_y$ are measurable with respect to $\mathcal{G}_{y, +}^{(x,m)}$ and $\mathcal{H}_{y, -}^{(x,m)}$.  $\mathcal{H}_{y, -}^{(x,m)}$ has an identity similar to \eqref{eq: recursive formula for upcrossings} for the BLP $\zeta$, and is $\mathcal{H}_{y, +}^{(x,m)}$- measurable but not $\mathcal{H}_{y, +}^{(x,m)}$- measurable. The argument is standard. \TBD \edt{The argument works for $x\geq 0$ but the identity should be modified slightly for $\Delta$, due to \eqref{eq: source of inhomogeneity}}
