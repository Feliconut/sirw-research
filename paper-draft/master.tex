...
\[
	\mathcal{H}_{y, +}^{(x,m)} = \sigma\left(\left\{\mathcal{F}_{m}^{\mathcal{B}, \mathcal{R}^{(x)}}\right\}  \cup \left\{\tau_{k, z}^{B} : x < z \le y, 0 \le k \le \mathcal{E}_{z-1, +}^{(x,m)} \right\} \right) 
.\] 
\comment{Iterative Construction}
