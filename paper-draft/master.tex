...
\begin{align}
	G_{n,K}^{(x,m)} :=  \qquad
		...
		& \bigcap_{y = x}^{K \sqrt{n}}  \left\{\color{red}\mathcal{E}_y^{(x,m)} < K^2 \sqrt{ n} \right\}  \cap \\
...
\begin{proof}
	...
	\[
		\lim_{K \to \infty} 
		P\left( \bigcap_{y = x}^{K \sqrt{n} } \left\{\mathcal{E}_y^{(x,m)} < K^2 \sqrt{ n} \right\} | A_K^{(x,m)}\right) = 1
	.\] 
