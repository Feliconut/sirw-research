...
\comment{Did not mention ``martingale''.}
	...
	\comment{Grammar: BLP or BLPs? Past tense or present tense? }
...
For example, in \cite{T96}, T\'oth also proved local limit theorems for the position of a random walker at independent geometric times with means of linear growth. 
	...
	\comment{Maybe we do not need to state the ``same'' theorem twice.}
...
.\]
	...
	\begin{lemma}(Lemma 2.2 \cite{KMP23})\label{lm: number of rarely visit sites}
		...
		$$
		\lim_{n\to\infty} P\left(\sup_{k\leq\lfloor nt \rfloor}  \sum_{x\in [I^X_{k-1}, S^X_{k-1}]} \mathbb{1}_{\{ L(x,k-1) \leq M \}} \geq 4n^b \right) = 0.
		$$
...
\begin{align}
	...
	\begin{equation}
		\label{eqn:goodgood}
		G_{n, K, t}\cap \{ \lambda_{x,m} \leq\lfloor nt \rfloor \} \subset   \bigcap_{y>x}^{K\sqrt{n}} G_{n, K}^{(x,m)}(y) \subset   \bigcap_{y>x}^{K\sqrt{n}} G_{n, K^2}^{(x,m)}(y)
		.\end{equation} 
...
%		&+ \sum_{y > x} \rho_y^{(x,m)} - \gamma
	...
	\begin{proof}[Proof of Lemma~\ref{lm: approx local drift by conditional means}]
		We first control \eqref{eqn:tempered-difference-0} on $G_{n, K, t} \cap \{\lambda_{x,m} \leq\lfloor nt \rfloor \}$. From \eqref{eqn:goodgood} and that $G_{n,K}^{(x,m)}(y)$ is increasing in $K$, we get that  \eqref{eqn:tempered-difference-1} is zero on $G_{n, K, t} \cap \{\lambda_{x,m} \leq\lfloor nt \rfloor \}$. 
%		Lemma~\ref{lm: approximation of means of local drift} has controlled \eqref{eqn:tempered-difference-4}. 
		...
		$
		 on $G_{n, K, t} \cap \{\lambda_{x,m} \leq\lfloor nt \rfloor \}$. By Azuma's inequality and Lemma~\ref{lm:lipchitz-bound-on-good-event}, we get that for any $\varepsilon>0$,
		\begin{align}
...
%		In both cases, the right hand side vanish when we first take $n $ to infinity, and then take $K$ to infinity.  
		...
		It remains to control  \eqref{eqn:tempered-difference-3} on $G_{n, K, t} \cap \{\lambda_{x,m} \leq\lfloor nt \rfloor \}$:
		\begin{align*}
			\sum_{y > x} \tilde\rho_y^{(x,m)} - \rho_y^{(x,m)}
		...
		Therefore, on $G_{n,K,t} \cap \{ \lambda_{x,m} \leq\lfloor nt \rfloor \}$,
	...
	\begin{align*}
		...
		P\left( \left| \sum_{y \ge x} \Delta_y^{(x,m)} - \rho_y^{(x,m)} \right|  > \varepsilon \sqrt{n} , G_{n,K,t}\cap \{\lambda_{x,m} \leq\lfloor nt \rfloor \}  \right) \\
