...
%	\edt{Consider changing both propositions to a version for $(x,m)$. This allows us to unify the part of the proof where we go from $(X_k, L(X_k, k))$ to $(x,m)$.}
	\edt{General discussion involving good events, but not too much technical things; postpone those to sect. 4}
...
\comment{Maybe discuss here that the diffusion approximation involves $\sigma_0^{\tilde \zeta}$ instead of $\sigma_{\varepsilon n}^{\tilde \zeta}$ as in \cite{KP16}, greatly simplifying the proof of equivalent of (Lemma~3.5, \cite{KP16}. }
...
\begin{align}
	B_{n, K} := \qquad
		...
		\bigcap_{i = 1}^{K \sqrt{n} } 
		\left\{\left| \tau_i^{\mathcal{B}, y} - 2 i \right| < \sqrt{ i } \log^2 n \right\}  \cap \\
		\label{eqn:good-event-4}
		& \bigcap_{y = - K \sqrt{n} }^{K \sqrt{n}} 
		\bigcap_{i = 0}^{K \sqrt{n} } 
		\left\{\left| \tau_{i+1}^{\mathcal{B}, y} - \tau_i^{\mathcal{B}, y} \right| < \log^2 n \right\}  
.\end{align}
...
\begin{align*}
	\overline{B} _{n, K} := 
& \left\{\sup _{k \le \lambda_{0, \left\lfloor K^2 \sqrt{n} \right\rfloor }} |X_k| < K^3 \sqrt{n} \right\} \cap \\
	&\bigcap_{y \in \mathbb{Z}} 
	\left\{L\left( y, \lambda_{0, \left\lfloor K^2 \sqrt{n} \right\rfloor } \right) < K^3 \sqrt{n} \right\}  \cap  \\
	& \bigcap_{|y| < K \sqrt{n} }  
	\left\{L\left( y, \lambda_{0, \left\lfloor K^2 \sqrt{n} \right\rfloor } \right) > K \sqrt{n} \right\}
.\end{align*}
\begin{align*}
	\overline{G}_{n, K} := 
		\overline{B} _{n, K} \cap 
	 B_{n, K^3}
.\end{align*}
...
\begin{align}
	G_{n,K}^{(x,m)}(y) :=  \qquad
	\label{eqn:good-event-5}
		& \left\{ L(y,  \lambda_{x, m}  ) < K^3 \sqrt{n} \right\} \cap \\
	\label{eqn:good-event-6}
		& \bigcap_{i = 1}^{L(y, \lambda_{x, m}) } \left\{\left| \tau_i^{\mathcal{B}, y} - 2 i \right| < \sqrt{ i } \log^2 n \right\}  \cap \\
	\label{eqn:good-event-7}
		& \bigcap_{i = 1}^{L(y, \lambda_{x, m}) } \left\{\left| \tau_{i+1}^{\mathcal{B},y} - \tau_i^{\mathcal{B},y} \right| < \log^2 n \right\}  
.\end{align}
...
\[
		\lim_{K \to \infty } \limsup_{n \to \infty } 
		P(\overline{G} ^c_{n, K}) = 0
	.\]
	and 
	\begin{equation}
		\label{eqn:goodgood}
		\overline{G} _{n, K} \subset 
		\bigcap_{|x|, m < K \sqrt{n} } \bigcap_{y \in \mathbb{Z}} G_{n, K}^{(x,m)}(y)
	.\end{equation}
\end{lemma}
\begin{proof}
	Following similar arguments in the previous lemma, we conclude that
	\[
		\lim_{K \to \infty } \limsup_{n \to \infty } P\left(B_{n, K^3}^c\right) = 0
	\]
	.
	To prove the other two events, observe that due to construction of BLP and symmetry around site $0$,
	\begin{align*}
	P\left( \overline{B}  _{n, K} \right) 
	&= 2 P_{K^{2} \sqrt{n} }^{\tilde \zeta}\left( \sup_{i \in \mathbb{N}} \tilde \zeta_i < K^3 \sqrt{n},
	\sigma_{K \sqrt{n} } > K \sqrt{n} , \sigma_{0} < K^3 \sqrt{n}   \right) 
	%&= 2 P_{K^{2} \sqrt{n} }^{\tilde \zeta}\left( \sup_{i \in \mathbb{N}} \tilde \zeta_i < K^3 \sqrt{n}  \right)  
	%+ 2 P_{K^{2} \sqrt{n} }^{\tilde \zeta}\left( \sigma_{K \sqrt{n} } > K \sqrt{n} , \sigma_{0} < K^3 \sqrt{n}   \right) 
	.\end{align*}
	Using the diffusion approximation of BLP, and scaling property of $Z^{2 \gamma}$, we can conclude that $P\left( \overline{B} _{n, K}^c \right) $ goes to $0$ when we first take $n$ to infinity and then take $K$ to infinity.

	To verify \eqref{eqn:goodgood}, note that, by monotonicity of local time process, under event $\overline{G} _{n, K}$, for any $|x|, m < K \sqrt{n}$, the local time process $L\left( y, \lambda_{x, m} \right) $ is dominated by $L\left( y, \lambda_{0, \left\lfloor K^2 \sqrt{n} \right\rfloor } \right) $. This proves \eqref{eqn:good-event-5}. Finally, note that the control $L(y, \lambda_{x, m}) < K^3 \sqrt{n} $ and $B_{n, K^3}$ gives \eqref{eqn:good-event-6} and \eqref{eqn:good-event-7}.
\end{proof}
...
%		(\comment{maybe $ y>x>0 $ currently only one side is dealt}) 
		...
		\begin{proof} 
			...
			\[
			\mathbb{E}\left[ \mu(m) \right] = 
			\mathbb{E}\left[ \mu(m)\mathbb{1}_{\{\mu(m)\geq 1 \} } \right] 
			\leq  \frac{1}{\inf_{i \in \mathbb{N}} 1/r(i) }\mathbb{E}\left[  \sum_{j=0}^{ \mu(m)-1 } \frac{1}{r(j)}   \right] <\infty, 
		\] 
...
%		
		...
		\begin{lemma}
			...
			$$
			\lim_{n\to\infty} P\left( \sup_{k\leq n t}  \abs{  	\sum_{y\in \left[X_{k}+1 ,S_{k}^X\right]} \left( \rho^{(X_k,L(X_k,k))}_y -  \gamma \cdot sgn(y) \right) } \geq  \varepsilon \sqrt{n}     \right) =0.
			$$
			Furthermore,
		\[
	\lim_{n\to\infty} P\left( \sup_{k\leq n t}  \abs{  	\sum_{y\in \left[I_k^{X} ,S_{k}^X\right]} \left( \rho^{(X_k,L(X_k,k))}_y -  \gamma \cdot sgn(y) \right) } \geq  \varepsilon \sqrt{n}     \right) =0.
	\]
\end{lemma}
...
%(it was)	\eqref{eq: mean of discrepancies for left sites}
	...
	Let $t>0$, and $b \in [\frac{\gamma \vee 0 }{2},\frac{1}{2})$.  For any $n,K,M>0$, we consider two types of events corresponding to control of extrema and rarely visited sites:
	\begin{align*}
...
$$
F_{n,\varepsilon}:= \left\{ \sup_{k\leq n t}  \abs{  	\sum_{y\in [X_{k}+1 ,M_k]} \left( \rho^{(X_k,L(X_k,k))}_y -  \gamma \cdot sgn(y) \right) } \geq  \varepsilon \sqrt{n}    \right \}$$ is contained in $A_{n,K} \cup B_{n,M} $ for some finite $K, M$ independent of $n$:   
...
In \eqref{eq: control of martingale difference for local drift}, for a fixed $(x,m)$, the inner sum is a martingale. To control the sum, we compare the martingale $\sum_y \Delta_y^{(x,m)} - \rho_y^{(x,m)}$ to a tempered version $\sum_{y > x} \tilde \Delta_{y}^{(x,m)} - \tilde\rho_y^{(x,m)}$ that has bounded increments.
\[
...
\begin{proof}[Proof of Lemma~\ref{lm: approx local drift by conditional means}]
	For arbitrary $|x| , m < K \sqrt{n} $, we can control \eqref{eqn:tempered-difference-0} on $G_{n, K, t} \cap \overline{G} _{n, K}$.
...
To control \eqref{eqn:tempered-difference-2} on $\overline{G} _{n, K}$, Azuma's inequality and Lemma~\ref{lm:lipchitz-bound-on-good-event} imply for arbitrary $\varepsilon>0$,
\begin{align}
	P\left( \left| \sum_{y = x + 1}^{K^3 \sqrt{n} } (\tilde\Delta_y^{(x,m)} - \tilde\rho_y^{(x,m)}) \right| > \varepsilon \sqrt{n}  \right) 
...
It remains to control  \eqref{eqn:tempered-difference-3} on $\overline{G} _{n, K}$. In view of \ref{eqn:goodgood},
	\begin{align*}
		&\mathbf{1}\left( \overline{G} _{n, K} \right) 
	\left| 	\sum_{y = x + 1}^{K^3 \sqrt{n} } \tilde\rho_y^{(x,m)} - \rho_y^{(x,m)} \right| \\
		&= \mathbf{1}\left( \overline{G} _{n, K} \right)
		\left| \sum_{y = x + 1}^{K^3 \sqrt{n} } \mathbb{E}\left[ \Delta_y^{(x,m)}\mathbf{1}\left( G_{n, K^2}^{(x,m)}(y) \right) - \Delta_{y}^{(x,m)} \middle| \mathcal{H}_{y-1}^{(x,m)}  \right] \right|   \\
		&= \mathbf{1}\left( \overline{G} _{n, K} \right)
		\left| \sum_{y = x + 1}^{K^3 \sqrt{n} } \mathbb{E}\left[ \Delta_y^{(x,m)}\mathbf{1}\left( \left( G_{n, K^2}^{(x,m)}(y) \right) ^c \right) \middle| \mathcal{H}_{y-1}^{(x,m)}  \right] \right|   \\
		&\le \left|\sum_{y = x + 1}^{K^3 \sqrt{n} }  \mathbf{1}\left(G_{n, K}^{(x,m)}(y-1)\right) \mathbb{E}\left[ \Delta_y^{(x,m)}\mathbf{1}\left( \left( G_{n, K^2}^{(x,m)}(y) \right) ^c \right) \middle| \mathcal{H}_{y-1}^{(x,m)}  \right] \right|  
	.\end{align*}
By monotonicity of BLP with respect to its initial conditions, we have the bound
\begin{multline*}
	\mathbf{1}\left( \overline{G} _{n, K} \right) \left| \sum_{y = x + 1}^{K^3 \sqrt{n}}  \tilde\rho_y^{(x,m)} - \rho_y^{(x,m)} \right| \le \\
	K^3 \sqrt{n} 
	\sqrt{ P\left( \left( G^{(x,m)}_{n, K^2}(y+1) \right) ^{c} \middle| \mathcal{E}_{y,+}^{(x,m)} = K^3 \sqrt{n}  \right) }
	\sqrt{ \mathbb{E}\left[ \left(\Delta_{y+1}^{(x,m)}\right)^2 \mathbf{1}\left( \mathcal{E}_{y,+}^{(x,m)} = K^3 \sqrt{n}\right)  \right]}
.\end{multline*}
To control the first probability, write $l = L\left( y+1, \lambda_{x, m} \right) $, and note that the probability is bounded by 
\[
	P\left(\mathcal{E}_{y+1,+}^{(x,m)} > K^6 \sqrt{n} \middle| \mathcal{E}_{y,+}^{(x, m)} = K^3 \sqrt{n}\right)
	= P\left(\mathcal{B}^{(y+1)}_{l} > K^6 \sqrt{n} \middle| \mathcal{R}^{(y + 1)}_l = K^3 \sqrt{n}  \right)
.\] 
...
\begin{align*}
	P\left(\mathcal{E}_{y+1,+}^{(x,m)} > K^6 \sqrt{n} \middle| \mathcal{E}_{y,+}^{(x, m)} = K^3 \sqrt{n}\right)
	%&\le P\left( \text{Binom}(K^6 \sqrt{n}, q ) < K^3 \sqrt{n}  \right)  \\
	%\intertext{By Hoeffding bound,}
	&\le \exp\left( - 2 K^6 \sqrt{n}(q - \frac{1}{K^3})  \right) 
,
\end{align*}
which converges to $0$ exponentially fast in both $K$ and $n$. Therefore, we need to bound expectation of $\left( \Delta_{y+1}^{(x,m)} \right) ^2$.  In view of Lemma~\ref{lm: concentration inequality}, we have
\begin{align*}
	\mathbb{E}\left[ \left(\Delta_{y+1}^{(x,m)}\right)^2 \mathbf{1}\left( \mathcal{E}_{y,+}^{(x,m)} = K^3 \sqrt{n}\right)  \right]
	&\leq \sum_{m = 0}^{\infty} \left( K^3 \sqrt{n}  + m \right) ^2 P\left( \left| \mathcal{E}_{y, +}^{(x,m)} - \mathcal{E}_{y+1, +}^{(x,m)} \right| > m \right)  \\
	&\leq \sum_{m = 0}^{\infty} \left( K^3 \sqrt{n}  + m \right) ^2 C e^{\frac{- c m^2}{ m \vee  K^3 \sqrt{n} }}  \\
	& < C' K^6 n 
.\end{align*}

Therefore, on the event $\overline{G} _{n, K}$, we have
\[
\left| \sum_{y > x} \tilde\rho_y^{(x,m)} - \rho_y^{(x,m)} \right| 
\le C' K^6 n \exp\left( - K^6 \sqrt{n}(q - \frac{1}{K^3}) \right) 
.\]
...
Finally, we control $\sup_{k < \left\lfloor nt \right\rfloor} \left| \sum_{y > X_k} \Delta_y^{\left(X_k,L(X_k, k)\right)} - \rho_y^{\left(X_k,L(X_k, k)\right)} \right|$ on the event $G_{n, K, t} \cap  \overline{G} _{n, K}$. First note that each $k < \left\lfloor nt \right\rfloor$ corresponds to some $x , m$ such that $|x| , m < K \sqrt{n} $ and $k = \lambda_{x, m}, X_k = x$, thanks to the event $G_{n, K, t}$. Also note from \eqref{eqn:goodgood} that for all $|x| , m < K \sqrt{n} $ we have $G_{n, K}^{(x,m)}(y)$ for all $y \ge x$, thanks to the event $\overline{G} _{n, K}$.
\begin{align*}
	...
	&\le P(G_{n, K, t}^c) + P(\overline{G} _{n, K}^c) +  P\left( \sup _{|x|, m < K \sqrt{n} } \left| \sum_{y > x} \Delta_y^{(x,m)} - \rho_y^{(x,m)} \right|  > \varepsilon \sqrt{n} , \bigcap_{y \ge  x} G_{n, K}^{(x,m)}(y) \right) \\
	&\le P(G_{n, K, t}^c) + P(\overline{G} _{n, K}^c) + K^2 n \sup _{|x|, m \le  K \sqrt{n} }
	P\left( \left| \sum_{y \ge X_k} \Delta_y^{(x,m)} - \rho_y^{(x,m)} \right|  > \varepsilon \sqrt{n} , \bigcap_{y \ge  x} G_{n, K}^{(x,m)}(y) \right) \\
	&\le P(G_{n, K, t}^c) + P(\overline{G} _{n, K}^c) + K^2 n \left( \exp\left( - C_{K, \varepsilon} \, n^{p } \log^{-4} n \right) + \exp\left( - C_{K, \varepsilon} \, \sqrt{n}  \log^{-5} n \right) \right) 
.\end{align*}
