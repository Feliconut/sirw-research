...
% % Modified 2024_02_03
% % Lastest Edition, 2024_02_03

% % (1) Changes responding to comments
% % (2) Modifications and double check around /comment and /edt
% % 
% % Changes to be made:  
% % (1) Removing comments/TBD, and double check
...
\begin{document}
	...
	\begin{abstract}
		...
		The approach is to approximate the total drift experienced by the walker via analyzing directed edge local times, described by the branching-like processes. The analysis depends on the diffusive approximation of the branching-like processes obtained in the Ray-Knight type framework.
...
%	For a precise definition of the probability measures and related $\sigma$-algebras, see \cite{KMP23}.
	...
	$\left(\mathcal{B}_{i}^{(y)},\mathcal{R}_{i}^{(y)} \right)$ is the state of a (generalized) P\'olya urn, after the $i$-th draw made from the urn. This is the reason why we define $\lambda_{y, 0}$ as the first time the walk reaches $y$: this is right before the first draw is made from the urn. Also, we note that $\left(\mathcal{B}_{i}^{(y)},\mathcal{R}_{i}^{(y)} \right)$ defined in \eqref{eq: RW to GPU} depends only on $\text{sgn}(y)$.
	...
	\begin{equation}\label{eq: generalized weights}
		...
		\end{cases}.
	\end{equation}
	For simplicity of notation, we write $\tau_{k,y}^{\mathcal{B}}$ in place of $\tau_k^{\mathcal{B}^{(y)}}$, and $\tau_{k,y}^{\mathcal{R}}$ in place of $\tau_k^{\mathcal{R}^{(y)}}$.
	...
	For $\zeta= \left(\mathcal{E}^{(x,m)}_{x-k,-} \right)_{k\geq 0}$, we get similar statements by reversing the roles of $+$ and $-$, as well as $\mathcal{B}$ and $\mathcal{R}$. $\zeta$ is inhomogeneous because of \eqref{eq: source of inhomogeneity}, and we have the transition probabilities depending on the sign of $x-k$: for $i,j\geq 0$
	\begin{equation}\label{eq: transition prob on negative}
		...
		\end{cases}
	\end{equation}
	where the generalized weights correspond to the case when $y>0$, $y=0$ and $y<0$ from \eqref{eq: generalized weights}. \comment{We note that both transition probabilities \eqref{eq: transition prob on positive} and \eqref{eq: transition prob on negative} agree on $[x,\infty)$.}
	...
	\begin{proof} 
		...
		\begin{equation*} 
			\rho_{y}^{(x,m)} 
			= \mathbb{E}\left[ \sum_{l=0 }^{ k -1  }  D\left(\tau^{\mathcal{B}}_{l,y}\,\,,\,\tau^{\mathcal{B}}_{l+1,y} \right) \middle| \mathcal{E}^{(x,m)}_{y-1,+} = k + \mathbb{1}_{\left\{1\leq y\leq x\right\}} \right]	
			= \mathbb{E}\left[ \sum_{l=0}^{k-1}  \mathcal{D}_{\tau^{\mathcal{B}}_{l+1,y}} -\mathcal{D}_{\tau^{\mathcal{B}}_{l,y}} \right]  
			= \mathbb{E}\left[  \mathcal{D}_{\tau^{\mathcal{B}}_{k,y}} \right]
		.\end{equation*}
	\end{proof}
	...
	\begin{proof}
		...
		\begin{align*}
			...
			\mathbb{P}\left( \mathcal{R}^{(0)} _{K \sqrt{n} } < \sqrt{K n}  \right) 
			.\end{align*}
...
%			+ \frac{1}{\left(2 i\right)^{p + 1}} 
	...
	\begin{proof}[Proof of Lemma~\ref{lm: approx local drift by conditional means}]
		...
		\begin{multline*}
			...
			.\end{multline*}
		\comment{We modify the argument to avoid the case when $x\leq 0$.}
...
%		
		\begin{align*}
			...
			,
		\end{align*}
		which converges to $0$ exponentially fast in $K \sqrt{n}$. Similarly, as $\abs{\Delta_{x+1}^{(x,m)}} \leq  \mathcal{E}_{x,+}^{(x,m)}+1 + \mathcal{E}_{x+1,+}^{(x,m)}$,  $ \left(\Delta_{x+1}^{(x,m)} \right)^2$ is bounded by at most $ (\mathcal{E}_{x,+}^{(x,m)}+1)^2$ products, we get that
		\begin{align*}
