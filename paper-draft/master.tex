...
% % Modified  2023_08_31
% % Lastest Edition, 2023_08_31
% % Description of changes:
% %  Major changes are in section 2, step 1 and subsection 3.1: 
% % 1) some changes in after 2.1 is updated 
% % 2) argument for (3.5) is updated. 
% % 
% % Changes to be made: 
% % (1) Section 2, and subsction 3.2: 
% %  1) Motivations for 2.2 for 2.2;
% %  2) description of construction of the BLP process. Constructions for x<0 case. 
...
\newcommand{\edt}[1]{\textcolor{red}{#1}} %edit time 08/31/2023
...
%		the accumulated local drifts at site $x$ by $m$-th visit, $$\delta_{x,m}:= \sum_{i=0}^\infty E[X_{i+1}-X_i\vert \mathcal{F}_{i}^X] \mathbb{1}_{\{X_i=x, L(x,i)\leq m\}};$$ 
	...
	\[
	   ...
	   \]
	   which is a Markov process with an initial value $(0,0)$. \comment{We can use $k$ only in the context of $X_k$.} Intuitively, $\left(\mathcal{B}_{y,i},\mathcal{R}_{y,i} \right)$ is the state a (generalized) P\'lya's urn, after the $i$-th draw made from the urn. This is the reason that we define $\lambda_{y, 0}$ to be the first time the walk reaches $y$ : this is right before the first draw be made from the urn. 
	...
	Therefore,  $\Delta_{y}^{(x,m)}$ is a sum of $\mathcal{E}^{(x,m)}_{y-1,+} $ increments which (jointly) depend only on $ \left(\tau^{B}_{l+1,y}: 0\leq l\leq   \mathcal{E}^{(x,m)}_{y-1,+}-1 \right) $
			...
			\end{equation}			
	From \eqref{eq: conditional increment}, $\abs{  D(\tau^{B}_{l,y},\tau^{B}_{l+1,y} )} \leq  \tau^{B}_{l+1,y}-\tau^{B}_{l,y}$ is stochastically dominated by a geometric random variable with mean independent of $l$. 
	The process $(\tau^B_{k,y})_{k\geq 0}$ is markov, and from the following subsection, $ (\mathcal{E}^{(x,m)}_{x+k})_{k\geq 0} $ is also markov \TBD. Then, on the event that $ \left\{ \mathcal{E}^{(x,m)}_{y-1,+} = k \right\} = \left\{ \mathcal{E}^{(x,m)}_{y,-} = k  \right\}$, $ \rho_{y}^{(x,m)} $ equals
	\begin{align}\label{eq: conditional mean in GPU represenetation}
		\rho_{y}^{(x,m)} = \mathbb{E}\left[\Delta_{y}^{(x,m)} \vert \mathcal{G}_{y-1}^{(x,m)} \right] 
		=& \mathbb{E}\left[ \sum_{l=0 }^{ k -1  }  D(\tau^{B}_{l,y},\tau^{B}_{l+1,y} ) \vert \mathcal{E}^{(x,m)}_{y-1,+} \right] 
		\notag \\
		=&	\mathbb{E}\left[ \sum_{l=0}^{k-1}  \mathcal{D}_{\tau^{B}_{l+1,y}} -\mathcal{D}_{\tau^{B}_{l,y}} \right]  
		= \mathbb{E}\left[  \mathcal{D}_{\tau^{B}_{k,y}} \right],
	\end{align} 
	where we use \eqref{eq: conditional increment} for the second and third equality. Expressions like \eqref{eq: cummulated drift at a site} and \eqref{eq: conditional mean in GPU represenetation} help us simplify the analysis of $\Delta_{y}^{(x,m)}$ and $\rho_{y}^{(x,m)}$, as will be shown section \ref{sec: approximations}. To analyze events like $ L(y,\lambda_{x,m}) = k+k'$ and $\mathcal{E}^{(x,m)}_{y-1,+} = k $, the branching-like processes play a key role.
		...
		\edt{More precisely, for an SIRW $(X_n)_{n\geq 0}$ with weight $w(.)$, and for any integers $x,m$ with $m\geq 0$ at the stopping time $\lambda_{x,m}$, the local times $\tilde{\zeta}= \left(\mathcal{E}^{(x,m)}_{x+k,+} \right)_{k\geq 0}$, and ${\zeta}= \left(\mathcal{E}^{(x,m)}_{x-k,-} \right)_{k\leq 0}$ are two Markov processes on $\mathbb{N}\cup\{0\}$, whose transition probabilities are related to the generalized weights in \eqref{eq: generalized weights}.} 
		The transition probabilities can be derived from a correspondence from one dimensional random walk and a branching process. \TBD Formally, we can project the recurrent random walk on $\mathbb{Z}$ (up to any finite time) to a tree by considering its moves starting from its first visit to site $x$, $\lambda_{x,0}$. It is convenient to consider only sites $y\geq x $ for the moment, since $y\leq x$ can be treated in a symmetric way, and we get concatenate two Markov processes to get a new one. Then we work on the case for $x \geq 0$, and leaving the case for $x <0 $ (for the next update). \TBD 
		The site $x \geq 0$ can be viewed as the root of the tree. Each right jump from a site $y\geq x$ gives a birth to a descendant in site $y+1$, while each left move from a site $y> x$ traces its ancestor at site $y-1$. 
		
		
		Then, depending on the sign of $x$, the difference $\mathcal{E}^{(x,m)}_{y,+} -\mathcal{E}^{(x,m)}_{y+1,-} $ depends on the relative position of $y$ with respect to $x$ and $0$ 
		\begin{equation}\label{eq: edge values}
			\mathcal{E}^{(x,m)}_{y,+} =\begin{cases}
				\mathcal{E}^{(x,m)}_{y+1,-} &\mbox{, for all $y\geq x$}\\
				\mathcal{E}^{(x,m)}_{y+1,-} +1 &,\mbox{ for all $0 \leq y\leq x$}
				\\
				\mathcal{E}^{(x,m)}_{y+1,-} &,\mbox{ for all $ y< 0 $}
			\end{cases}
		\end{equation}
