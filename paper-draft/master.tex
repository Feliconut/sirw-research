...
In the case $p \in (\frac{1}{2}, 1]$, the total drift accumulated at any site $x$ turns out to be absolutely summable, which allows one to estimate the total drift easily (\cite{KMP23}, Lemma~2.3-2.4).
	...
	\begin{enumerate}
		...
		\item The local time of a site $x \in \mathbb{Z}$ by time $j \in \mathbb{N}_0$, $L(x,j):= \sum_{i=0}^j \mathbb{1}_{\left\{X_i=x\right\} }
			.$
		...
		\[
		\mathcal{E}^j_{x,+} = \sum_{i=0}^{j-1} \mathbb{1}_{\left\{X_i=x, X_{i+1} =x+1 \right\} } ,
	\]
...
%(it was)	\eqref{eq: mean of discrepancies for left sites}
	...
	$$
	F_{n,\varepsilon}:= \left\{ \sup_{k\leq n t}  \abs{  	\sum_{y\in [X_{k}+1 ,S_k]} \left( \rho^{(X_k,L(X_k,k)-1)}_y -  \gamma \cdot sgn(y) \right) } \geq  \varepsilon \sqrt{n}    \right \right\}
	$$ 
