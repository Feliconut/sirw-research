...
%		the accumulated local drifts at site $x$ by $m$-th visit, $$\delta_{x,m}:= \sum_{i=0}^\infty E[X_{i+1}-X_i\vert \mathcal{F}_{i}^X] \mathbb{1}_{\{X_i=x, L(x,i)\leq m\}};$$ 
	...
	\[
	  ...
	  \begin{equation} \label{eq: RW to GPU}
	   ...
	   \end{equation}
	   which is a Markov process with an initial value $(0,0)$. \comment{We can use $k$ only in the context of $X_k$.} Intuitively, $\left(\mathcal{B}_{y,i},\mathcal{R}_{y,i} \right)$ is the state of a (generalized) P\'olya urn, after the $i$-th draw made from the urn. This is the reason that we define $\lambda_{y, 0}$ to be the first time the walk reaches $y$ : this is right before the first draw be made from the urn. 
	...
	\begin{equation}\label{eq: initial condition}
		...
		\end{cases} 
	\end{equation}	
	for values $y>0$, $y<0$ or $y=0$. Therefore, we get three types of transition probabilities
	...
	\begin{equation}\label{eq: generalized weights}
		...
		\end{cases}.
	\end{equation}
	For simplicity of notation, we drop the subscript $y$ when we talk about a fixed site of $y$ and when there's no confusion.
	...
	\end{equation} 
	 Expressions like \eqref{eq: cummulated drift at a site} and \eqref{eq: conditional mean in GPU represenetation} help us simplify the analysis of $\Delta_{y}^{(x,m)}$ and $\rho_{y}^{(x,m)}$, as will be shown section \ref{sec: approximations}. To analyze events like $ L(y,\lambda_{x,m}) = k+k'$ and $\mathcal{E}^{(x,m)}_{y-1,+} = k $, the branching-like processes play a key role.
		...
		\edt{More precisely, for an SIRW $(X_n)_{n\geq 0}$ with weight $w(.)$, any integers $x,m$ with $m\geq 0$, the local times at the stopping time $\lambda_{x,m}$, 
		\[
			\tilde{\zeta}= \left(\mathcal{E}^{(x,m)}_{x+k,+} \right)_{k\geq 0}, \quad {\zeta}= \left(\mathcal{E}^{(x,m)}_{x-k,-} \right)_{k\geq 0}
			\]
		...
		\edt{To derive the transition probabilities for $\tilde{\zeta}$ and ${\zeta}$ (given $x,m$), we assume that $x\geq 0$, while for $x<0$, the symmetry gives the following identities in distribution
		\[
		 \left(\mathcal{E}^{(x,m)}_{x+k,+} \right)_{k\geq 0} \overset{d}{=} \left(\mathcal{E}^{(-x,m)}_{-x-k,-} \right)_{k\geq 0}, \quad  \left(\mathcal{E}^{(x,m)}_{x-k,-} \right)_{k\geq 0} \overset{d}{=} \left(\mathcal{E}^{(-x,m)}_{-x+k,+} \right)_{k\geq 0}.
		\]
	} 
		\edt{
