...
% % Modified  2023_08_23
% % Lastest Edition, 2023_08_23
% % Description of changes: 
% % 1) minor changes are made for undirected edge: (a,b)-->  {a,b} . They are in page 1
% % 2) Notations for edge local times now has +, - to indicate directions:  \mathcal{E}^{j}_x -> \mathcal{E}^{j}_{x,+},  \mathcal{D}^{j}_x --> \mathcal{E}^{j}_{x,-}
% % 3) minor changes to the section 2: the change of the section name, the description of approximations of Gamma_n^+; 
% % the approximations of $\Delta_{y}^{(x,m)}$ by $sign(y)\cdot \gamma$  and equation (2.7) is updated also. The statements of propsitions 2.3 and 2.4 are kept the same. 
% % 
% % 
% % 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{document}
	...
	\begin{proposition}\label{lm: control of acc drift}
		...
		\end{equation}
	\end{proposition}
	To prove Proposition \ref{lm: control of acc drift}, we assume that $X_n=x $, and $L(x,n)=m$. We decompose the accumulated drift as $\Gamma_n = 	\Gamma_n^+ +	\Gamma_n^0 + \Gamma_n^-$ 
	\begin{align}
	...
	$$ which has $(M_n - X_n)$ terms. If we can further approximate terms $\Delta_y^{(x,m)}$ by $\gamma\cdot sgn(y)$ without generating a total error of size $\epsilon \sqrt{n}$, $\Gamma_n^+$ is approximately 
	$$   
	\gamma \cdot (M_n - \abs{X_n}).
	$$
	...
	\end{equation}
	To this end, we consider the filtration $\left(\mathcal{G}_{y}^{(x,m)}\right)_{y\geq x}$, where $ \mathcal{G}_{y}^{(x,m)} = \sigma\left( \mathcal{E}^{n}_{y,+} : y \geq x \right)$, and further approximate $\Delta_y^{(x,m)}$ by its conditional mean with respect to $\mathcal{G}_{y-1}^{(x,m)}$,
	...
	then the sequence 
	$$
	(\mathcal{B}_k,\mathcal{R}_k )_k:=\left(\mathcal{E}^{\lambda_{y,k}}_{y,-}, \mathcal{E}^{\lambda_{y,k}}_{y,+}\right)_{k\geq 0}  
...
Consider some site $x \neq 0$ and let $m > 0$. Let $n = \lambda_{x, m}$, so $X_n = x$. By symmetry we assume without loss of generality that $x >  0$. \TBD. (We are currently only considering sites right to $x$, i.e. $y \ge x$.) 
...
\begin{align}
	G_{n,K}^{(x,m)} :=  \qquad
		...
		& \bigcap_{y = x}^{K \sqrt{n}}  \left\{\color{red}\mathcal{E}_{y,+}^{(x,m)} < K^2 \sqrt{ n} \right\}  \cap \\
...
and we define
\[
	G_{n, K} := \bigcap_{|x|, |m| \le K \sqrt{n} } G_{n,K}^{(x,m)} 
.\] 

We later verify that $\lim _{K\to \infty } \limsup_{n \to \infty } G_{K} = 0$.

\TBD {\color{red} Big problem here. Definition of $G$ need to be fixed, and carefully redo the proof that $G$ vanishes.}

On the good event $G_{n,K}$ we can have a deterministic bound for $|\Delta_y^{(x,m)} - \rho_y^{(x,m)}|$ uniform in $y$, which makes the martingale in \eqref{eq: control of martingale difference for local drift} admissible.
...
Having this lemma, we can apply Azuma's inequality to the sum in \eqref{eq: control of martingale difference for local drift} on $G_K^{(x,m)}$. In the case $p < \frac{1}{2}$ we get for all $\varepsilon>0$,

\begin{align}
	P\left( \left| \sum_{y\ge X_n} (\Delta_y^{(x,m)} - \rho_y^{(x,m)})  \right| > \varepsilon \sqrt{n}  \right) \notag
	&\le \exp\left( - \frac{\varepsilon^2 n}{2 K \sqrt{n} \left( C_{K} n^{-\frac{1}{2}p + \frac{1}{4}} \log^2 n  \right)^2 } \right) + \left( 1 - P\left( G_K^{(x,m)} \right)  \right) \notag \\
	&= \exp\left( - C_{K, \varepsilon} \, n^{p } \log^{-4} n \right) +\left( 1 - P\left( G_K^{(x,m)} \right)  \right)   \label{eqn:azuma-drift-martingale}
.\end{align}
In the case $p = \frac{1}{2}$ the bound becomes
\begin{align}
	P\left( \left| \sum_{y \ge X_n} (\Delta_y^{(x,m)} - \rho_y^{(x,m)}) \right|  > \varepsilon \sqrt{n}  \right) \notag
	&\le  \exp\left( - C_{K, \varepsilon} \, \sqrt{n}  \log^{-5} n \right) +\left( 1 - P\left( G_K^{(x,m)} \right)  \right) 
.\end{align}
In both cases, the right hand side vanish when we first take $n $ to infinity, and then take $K$ to infinity. To bound $\sup_{k < nt} \left| \sum_{y \ge X_k} (\Delta_y^{(x,m)} - \rho_y^{(x,m)}) \right|$, we note that for every $k < nt$ there exists $(x,m)$ such that $x = X_k$ and $m = L_x(k)$. Thus, this amounts to bounding the supremum of $\left| \Delta_y^{(x,m)} - \rho_y^{(x,m)} \right| $ over all $(x,m)$ pairs. 

\TBD. Do the union bound.


\begin{proof}
	{\color{red} Proof that the event $G_K$ is good, need to be overhauled.}
	We want to verify that for each  $(x,m)$,
	\[
		\lim_{K\to \infty } \limsup_{n \to \infty } G_{n, K}^{(x,m)} = 1
	,\] 
	and moreover, this convergence is uniform in $(x,m)$, so that
	\[
		\lim_{K\to \infty } \limsup_{n \to \infty } G_{n, K} = 1
	.\] 

	From process-level tightness we know that probability of $A_K^{(x,m)}$ approaches $1$ as $K $ goes to $ \infty $. Now we control $B_K^{(x,m)}$ conditioned on $A_K^{(x,m)}$. We would like to show that
	\[
		\lim_{K \to \infty} 
		P\left( \bigcap_{y = x}^{K \sqrt{n} } \left\{\mathcal{E}_{y,+}^{(x,m)} < K^2 \sqrt{ n} \right\} | A_K^{(x,m)}\right) = 1
	.\] 
%Now assume that there exists $y$,  $x\le y\le K \sqrt{n} $, such that $\mathcal{E}_y^{(x,m)} \ge K^2 \sqrt{n} $. On $A_K^{(x,m)}$ we have $\mathcal{E}_{K \sqrt{n} }^{(x,m)} = 0$, so pigeonhole principle implies that there exists $z$, $x\le z\le K \sqrt{n} $, such that $\mathcal{E}_z^{(x,m)} - \mathcal{E}_{z+1}^{(x,m)}\ge K$. DOES NOT WORK



To establish the likelihood of $C_K^{(x,m)}$ and $D_K^{(x,m)}$, note that the inner intersections over $i$ give rise to events that are i.i.d. over $y$. 
\begin{align*}
	P\left(\left( C_{K}^{(x,m)}\right)^c\right) 
...
.\end{align*}
Where in the second line we used Lemma~\ref{lm: concentration inequality} and took supremum over $y$. For fixed $K$, the last line goes to $0$ as $n$ goes to infinity. For \eqref{eqn:good-event-4}, we make a similar argument and note that, since the probability that the next ball drawn is blue is bounded below by some constant $q > 0$, $\tau_{i+1}^{\mathcal{B}} - \tau_{i}^{\mathcal{B}}$ is stochastically dominated by some geometric random variable $\text{Geo}(q)$. Hence

\[
	P\left(\left( D_{K}^{(x,m)}\right)^c\right) 
...
\begin{align*}
	...
	\intertext{\TBD need to fix the number of logs.}
