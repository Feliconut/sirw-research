...
% % Modified  2023_12_18
% % Lastest Edition, 2023_12_18

% % (1) Minor change in the abstrac to avoid "we consider" at the start of both Abstract and Introduction)
% % (2) Discussion of Ray-Knight approaches in abstract 
% % (3) Minor changes for typos. 
% % 
% % Changes to be made:  
% % (1) Issues in Introduction (acoording to Tom/ John's comments)
% % (2) Removing comments/TBD, and double check
...
% We can omit the last sentence of the abstract	
	\begin{abstract}
We show the convergence of a one-dimensional self-interacting random walk to a Brownian motion perturbed at extrema under the diffusive scaling. This completes the functional limit theorem in \cite{KMP23} for the asymptotically free case when $0<p \leq \frac{1}{2}$. The approach is to carefully approximate the local drifts of the random walk via studying the directed edge local times, described by branching-like processes and generalized Ray-Knight Theorems. (We construct certain events where martingales associated to the branching-like processes are well behaved.)
	\end{abstract}


%
%We consider a one-dimensional non-Markovs self-interacting random walk, and complete the functional limit theorem in \cite{KMP23} for the asymptotically free case with $0<p \leq \frac{1}{2}$. The approach is to carefully approximate the local drifts of the random walk via studying the directed edge local times, described by branching-like processes and generalized Ray-Knight Theorems. It turns out that certain martingales associated to these branching-like processes are well behaved. (Or we construct events such that certain martingales associated to these branch-like processes are well behaved.)
	

	\section{Introduction}
	%The current project is a completion of Theorem 1.2 in \cite{KMP23}. 
	
	We consider a discrete time nearest neighbor self-interacting random walk (SIRW) $(X_k)_{k\geq 0}$ on $\mathbb{Z}$ as in \cite{T96,KMP23}. The walk $(X_k)_{k\geq 0}$ starts from $X_0 = 0$ and is non-Markov: its transition probability for $X_{k+1}$ depends on its history up to $X_k$, 
	\begin{align}
	...
	In this model, we require $w(.)$ to be monotone and converging to $1$, with regular polynomial asymptotic behavior,
	...
	\edt{
	A key element in T\'oth's analysis of SIRW is the connection between SIRW and its local time, which is in spirit, similar to how Ray-Knight theorem connects the local times of a Brownian motion with certain squared Bessel processes. By taking diffusion approximations of the local times processes for SIRW, squared Bessel processes of appropriate dimensions appears as the diffusion limits. These generalized Ray-Knight Theorems can be used to infer certain behaviors of scaling limits of the original SIRW.}
	...
	These local limit theorems imply that the Brownian motion perturbed at extrema (BMPE) (see Definition~\ref{defn:BMPE} below) is the only possible candidate for the weak limit of one-dimensional distributions of disffusively rescaled SIRWs if the limit exists. 
	\edt{Similar diffusion approximations have also been proved for other one dimensional non-Markov random walks, such as excited random walks (ERW), see \cite{KM11,KZ13,KP16,DK12,HLSH18,KMP22} for some recent references. Although the squared Bessel processes also arise as the limiting distribution, the behaviors of these ERWs can be very different from SIRWs, for example transience versus recurrence, limiting distributions as Brownian motions versus BMPE.}  
	Based on these result, in \cite{KMP23}, Kosygina, Mountford and Peterson showed the process-level convergence of rescaled asymptotically free SIRW to BMPE under an additional assumption that $p > \frac{1}{2}$. Interestingly, they also constructed a counterexample to the functional convergence of rescaled SIRW in the polynomially self-repelling case, which means that there is no general thereom to pass easily from local time processes to functional convergence of the walk. The current work focuses on the asymptotically free case, and serves as a completion of the functional convergence result in \cite{KMP23} by removing the assumption that  $p > \frac{1}{2}$.
	...
	\begin{theorem*}[\cite{KMP23}, Theorem~1.1]
			Let $w(.)$ be monotone and satisfy \eqref{eq: asymptotics of w} for $p\in (\frac{1}{2},1]$, and $\kappa >0 $, we have the following process-level convergence
	\[
...
The functional convergence of rescaled SIRW to BMPE has also been shown for other non-Markov one-dimensional random walks,
	...
	Our result should be compared to those recurrent ERWs in the non-boundary case, see \cite{KP16,KMP23}, because their properties and the approaches are very similar. 
	In the recurrent non-boundary case, the diffusive scaling is the correct scaling for both the walk and the local time profiles and BMPE is the limit expected (as observed in \cite{T96}), while a nontrivial scaling limit under a different scaling other than BMPE is expected when the walk is transient or in the boundary recurrent case. We also remark that $\gamma<1$ in our model implies that we are not in the boundary recurrent case.


	The analysis of local time process of random walk via \textit{Branching-like processes}, taking advantage of the tree structure of the walk's excursions in one dimension, is core to the analysis of both SIRW (considered in this work as well as \cite{T96, KMP23}) and ERW (considered in \cite{DK12, KP16, KMP22}). 
	...
	These two auxiliary processes are crucial for controlling total drift attained by the walk in both \cite{KMP23} and this work.
In the case $p \in (\frac{1}{2}, 1]$, the total drift accumulated at any site $x$ turns out to be absolutely summable, which allows one to estimate the total drift easily (\cite{KMP23}, Lemma~2.3-2.4).
	...
	It turns out that we can estimate accumulated drift without using absolute summability of local drifts, if we stop the walk at particular excursion times and express the total attained drift as a spatial martingale adapted to the natural filtration of the branching-like process. Then the martingale has bounded increments on an event with high probability, allowing us to sum the local drifts and obtain estimate for the walker's accumulated drift.
	...
	\begin{enumerate}
		\item The running maximum of $(X_i)_{i \ge 0}$ at time $k$, $S_k= \sup\left\{X_i : i \le k\right\}$; the running minimum at time $n$, $I_n= \inf \left\{X_i: i \le k\right\} $.
		\item The local time of a site $x \in \mathbb{Z}$ by time $j \in \mathbb{N}_0$, $L(x,j):= \sum_{i=0}^j \mathbb{1}_{\{X_i=x\} }
			.$
			% the definition for local time in \cite{KMP23} and \cite{KP16} are different by at most 1. The main difference comes from whether to count the last step $X_n$. For the former case, $L(x,\lambda_{x,m}) = m+1$, while $L(x,\lambda_{x,m}) =m$ for the latter case.  
	...
	\]
		...
		\mathcal{E}^j_{x,-} = \sum_{i=0}^{j-1} \mathbb{1}_{\{X_i=x, X_{i+1} =x-1 \} }.
	\]
		\item For any $m\geq 0$, the time of $m$-th return ($m+1$-th visit) to site $x$, is denoted by
			\[
				\lambda_{x,m} = \inf\left\{t \geq 0: L(x,t) = m+1 \right\}
		.\]
		We write $\mathcal{E}^{(x,m)}_{y, -}$ for $\mathcal{E}^{\lambda_{x,m}}_{y,-}$, and $\mathcal{E}^{(x,m)}_{y, +}$ for $\mathcal{E}^{\lambda_{x,m}}_{y,+}$.
	\end{enumerate}
	...
	\begin{equation}\label{eq: control of acc drift + }
		\lim_{n \to \infty }\mathbb{P}\left(\sup_{k\leq nt} \abs{\Gamma^+_k - \gamma \cdot \left(S_k - \abs{X_k} \right)   } > \varepsilon \sqrt{n}  \right) =0. 
	\end{equation}
	...
	\begin{equation}
		\lim_{n \to \infty }\mathbb{P}\left(\sup_{k\leq nt} \abs{\sum_{y> X_k} \left( \Delta_{y}^{(X_k,L(X_k,k) - 1)} - \gamma  \cdot sgn(y) \right)   }  > \varepsilon \sqrt{n}  \right) =0. 
	\end{equation}
	...
	\begin{lemma}\label{lm: approximation of means of local drift}
		...
		\begin{equation}\label{eq: control of expected local drift}
			\lim_{n \to \infty }\mathbb{P}\left(\sup_{k\leq nt} \abs{\sum_{y> X_k} \left( \rho_{y}^{(X_k,L(X_k,k)-1)} - \gamma  \cdot sgn(y) \right)   }  > \varepsilon \sqrt{n}  \right) =0. 
		\end{equation}
	\end{lemma}
	...
	\begin{lemma}\label{lm: approx local drift by conditional means}
		...
		\begin{equation}\label{eq: control of martingale difference for local drift}
			\lim_{n \to \infty }\mathbb{P}\left(\sup_{k\leq nt} \abs{\sum_{y> X_k} \left(\Delta_{y}^{(X_k,L(X_k,k)-1)}- \rho_{y}^{(X_k,L(X_k,k)-1)} \right)   }  > \varepsilon \sqrt{n}  \right) =0. 
		\end{equation}
	\end{lemma}
	...
	\[
	\lambda_{y,0} :=\inf\{ t\geq 0: X_t = y \} , \quad \lambda_{y,i+1} := \inf\{ t> \lambda_{y, i}: X_t = y \}.
	\] 
	...
	The following lemma says that $\Delta^{(x,m)}_{y}$ is $\mathcal{H}_{y, +}^{(x,m)}$- measurable but not $\mathcal{G}_{y, +}^{(x,m)}$- measurable, and we have two identities for $\Delta^{(x,m)}_{y}$ and  $\rho_{y}^{(x,m)}$. 
	...
	To facilitate our arguments in section \ref{sec: approximations}, we list some results from \cite{KMP23,T96}, whose proofs we omit at this moment.
...
.\]
	...
	The third result is about the diffusion approximations of the branching-like processes, and it follows from the Proposition A.3 in \cite{KMP23}. Its proof follows arguments from Toth \cite{T96}. A consequence of this result is the process level tightness of extrema, Proposition 2.1 \cite{KMP23}. 
...
\begin{proof}
	Consider the event that $L(y, n) > K \sqrt{n} $ for some  $y \in \mathbb{Z}, n, K > 0$. Define $m_{n, K } = \tau^{\mathcal{R}, 0}_{\sqrt{K n} }$. Now, either
	\begin{itemize}
		\item $L(y, \lambda_{0, m_{n, K}}) \ge L(y, n)$, in which case $\lambda_{0, m_{n, K}} \le  n$; or
		\item $L(y, \lambda_{0, m_{n, K}}) \ge  L(y, n)  > K \sqrt{n} $.
	\end{itemize}
	By symmetry we only need to control $P\left( \sup _{y \ge  0} L(y, n) > K \sqrt{n}  \right) $. In view of the case analysis above, we have
	\begin{align*}
...
.\end{align*} 
Both probabilities go to zero as we let $K$ go to infinity. 
\end{proof}
...
\section{Approximations of Accumulated Drift}\label{sec: approximations}
	...
	To simplify our argument, we first define typical events $G_{n, K, t}$ in the next section, and apply the typicality of $G_{n, K, t}$ in subsequent proofs.
...
\begin{proof}%[Proof of Lemma~\ref{lem:good-event}]
	...
	The remaining two events \eqref{eqn:good-event-3} and \eqref{eqn:good-event-4} encode the asymptotic behavior of Polya's Urn processes at each site $y$. To estimate the probability of event \eqref{eqn:good-event-3}, we use Lemma~\ref{lm: concentration inequality}. Specifically,
	\begin{align*}
...
On $G_{n, K, t}$, we can prove precise bounds of local drifts, which allows us to apply martingale concentration inequalities to estimate accumulated drift.
...
\begin{proof}[Proof of Lemma~\ref{lm: control of martingale}]
	...
	\begin{align*}
		...
		.
	\end{align*}
	In the cases $(x=0)$ and $(x < 0)$ we have exactly the same bound with minor difference in calculation.
	...
	on the event that $l = \mathcal{E}^{(x,m)}_{y-1,+} +\mathbb{1}_{\{1\leq y\leq x\}}$, we have \eqref{eq: conditional mean in GPU represenetation} 
	...
	\begin{proof} 
		...
		\begin{align}
			& \sum_{j=m}^{\infty} \left(\abs{\frac{1}{r(j)} -\frac{1}{r(m)} } \cdot \mathbb{1}_{\{\mu(m)\geq j \}}\right)\\
			& \leq  \sum_{0\leq j-m \leq A \sqrt{m}\log m } \left(\abs{\frac{1}{r(j)} -\frac{1}{r(m)} } \cdot \mathbb{1}_{\{\mu(m)\geq j \}}\right)
			...
			\quad +\quad 2\left(\sup_j \frac{1}{w(j)}\right) \cdot \sum_{j\geq m + A\sqrt{m}\log m } \mathbb{1}_{\{ \mu(m) > j \}}
...
2\left(\sup_j \frac{1}{w(j)}\right) \cdot \sum_{j\geq m + A\sqrt{m}\log m } \mathbb{1}_{\{ \mu(m) > j \}}
			\le & 2\left(\sup_j \frac{1}{w(j)}\right) \sum_{j-m \geq A \sqrt m \log m  } P( \mathcal{D}_{\tau^B_m} \geq j-m )  
			\notag 
			\\
...
%(it was)	\eqref{eq: mean of discrepancies for left sites}
	...
	Let $t>0$, and $b \in \left[\frac{\gamma \vee 0 }{2},\frac{1}{2}\right)$.  For any $n,K,M>0$, we consider two types of events corresponding to control of extrema and rarely visited sites:
	\begin{align*}
		A_{n,K}:=&\left\{ \min\{-I_{nt}, S_{nt}\} \geq K \sqrt{n}  \right\}
		\\
		B_{n,M}:=& \left\{  \sup_{k\leq n t} \sum_{ y\in (X_{k-1}, S_{k-1}]}  \mathbb{1}_{\{ \mathcal{E}^{k-1}_{y-1,+} \leq M  \}} >n^b  \right\}.
	\end{align*}
	...
	\begin{align*}
		...
		C(0) \cdot \sup_{k\leq n t} \left\{   	\sum_{y\in [X_{k}+1 ,S_k]} \mathbb{1}_{ \{ \mathcal{E}^{k-1}_{y-1,+} \leq M \} } \right\}
		\notag
		\\
		+& C(M) \cdot \sup_{k\leq n t} \left\{   	\sum_{y\in [X_{k}+1 ,S_k]} \mathbb{1}_{ \{ \mathcal{E}^{k-1}_{y-1,+} \geq M \} } \right\},
	...
	\end{equation} As $n$ goes to infinity, \eqref{eq: an upper bound on good set} is smaller than $\varepsilon \sqrt{n}$ for any $K>0$ and any $M$ with $C(M) < \frac{\varepsilon}{2K}$ . 
...
\label{sec:DeltaRho}
In this subsection, we prove Lemma \ref{lm: approx local drift by conditional means} and thuus complete the proof of Theorem \ref{thm: main}. This proof is similar to, and slightly more technical than the proof of (Lemma~4.2, \cite{KP16}). 
...
%		In both cases, the right hand side vanish when we first take $n $ to infinity, and then take $K$ to infinity.  
	...
	For $\sup_{k < nt} \left| \sum_{y > X_k} \Delta_y^{\left(X_k,L(X_k, k)\right)} - \rho_y^{\left(X_k,L(X_k, k)\right)} \right|$, we use a union bound by considering possible values $(x,m)$ for $\left(X_k, L\left(X_k, \left\lfloor nt  \right\rfloor - 1\right)\right)$. Note that if $G_{n,k,t}$ occurs, for any $k\leq nt$, there is a pair $(x,m)$ with $\lambda_{x,m}=k$ such that $\abs{x},m <K\sqrt{n}$. Therefore, we get from \eqref{eqn:azuma-drift-martingale} and \eqref{eq: difference of cond means} that 
	\begin{align*}
