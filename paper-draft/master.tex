...
\begin{document}
	...
	The following functional limit theorem is shown in [\cite{KMP23}]: assuming $p\in (\frac{1}{2},1]$, we have the following process-level convergence
	\[
...
In the case $p \in (\frac{1}{2}, 1]$, the total drift accumulated at any site $x$ turns out to be absolutely summable, which allows one to estimate the total drift easily ([\cite{KMP23}, Lemma~2.3-2.4]).
	...
	In \cite{KMP23}, the analysis of local behavior of the walk through Generalized P\"olya's urn processes were crucial in the control of total drift. 
	In the case $p \in (\frac{1}{2}, 1]$, the total drift accumulated at any site $x$ turns out to be absolutely summable, which allows one to estimate the total drift easily ([\cite{KMP23}, Lemma~2.3-2.4]).
...
%(it was)	\eqref{eq: mean of discrepancies for left sites}
	...
	$$
	F_{n,\varepsilon}:= \left\{ \sup_{k\leq n t}  \abs{  	\sum_{y\in [X_{k}+1 ,M_k]} \left( \rho^{(X_k,L(X_k,k))}_y -  \gamma \cdot sgn(y) \right) } \geq  \varepsilon \sqrt{n}    \right \}
	$$ 
